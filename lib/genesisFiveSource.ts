type GenesisFivePersonalSection = {
  chapter: number;
  startVerse: number;
  endVerse: number;
  reference: string;
...
export const GENESIS_FIVE_RAW_NOTES = String.raw`# 📖 Genesis 5:1–5
...
🌍 Humanity begins to multiply

🌱 The story continues through Seth
...
🌍 People reflect God uniquely

📖 Connects back to Genesis 1:26
...
👨‍👩‍👧 Humanity was meant to grow

💛 God’s favor was given
...
👶 Seth is born later in life

📖 Early lifespans were much longer
...
📖 God’s image passes through generations
...
📖 Not every child is named

🌱 The family line grows
...
👶 Before his first recorded son

📖 Long lifespans were common

🌍 Early humanity was still growing
...
👨‍👩‍👧 Saw many generations

📖 A remarkably long life

🌍 Humanity continued multiplying
...
👨‍👩‍👧 Multiple generations were born

🌍 Humanity continued spreading

📖 One of the longest lives in Scripture
...
⚠️ Death reaches every generation

📖 Sin's consequences remain

💔 Even long lives come to an end
...
⏳ Enos was 90 years old

👶 Before his first recorded son

📖 Long lifespans continued

🌍 Humanity was still multiplying
...
📖 One of the longest lives recorded

👨‍👩‍👧 Saw many descendants
...
⚠️ Death reaches everyone

📖 Sin's consequences remain

💔 Long life cannot stop death
...
📖 Not every descendant is listed
...
👨‍👩‍👧 Multiple generations witnessed
...
🙏 Relationship with God emphasized
...
📖 Different from Cain's Lamech

🌱 Family line continues

👨 Father of Noah to come
...
🏆 Longest lifespan in Scripture

📖 Nearly a thousand years

🌍 Witnessed many generations
...
🌍 An important generation begins
...
👨‍🌾 Hard labor continued

📖 Connects back to Genesis 3
...
👶 Before his recorded sons

📖 Preparing for the flood story
...
📖 The genealogy reaches Noah

🌊 The flood story is next

🌍 Humanity's future continues through his family
`;

type ParsedGenesisFiveSection = GenesisFivePersonalSection & {
...
function parseGenesisFiveRawNotes(raw: string): ParsedGenesisFiveSection[] {
  const lines = raw.replace(/\r\n/g, '\n').split('\n');
  const sections: ParsedGenesisFiveSection[] = [];

  let currentSection: ParsedGenesisFiveSection | null = null;
...
function buildGenesisFiveDeepNotes(sections: ParsedGenesisFiveSection[]) {
  const blocks = sections.map((section) => {
    const phrases = section.phrases
      .map(([heading, body]) => `### ${heading}\n\n${body}`)
      .join('\n\n---\n\n');

    return `# ${section.icon} ${section.displayReference}\n\n## ${section.title}\n\n${phrases}`;
  });

  return blocks.join('\n\n');
}

const parsedGenesisFiveSections = parseGenesisFiveRawNotes(GENESIS_FIVE_RAW_NOTES);

export const GENESIS_FIVE_PERSONAL_SECTIONS: GenesisFivePersonalSection[] = parsedGenesisFiveSections.map(({ displayReference, ...section }) => section);

export const GENESIS_FIVE_DEEP_NOTES = buildGenesisFiveDeepNotes(parsedGenesisFiveSections);
