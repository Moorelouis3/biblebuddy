 ...
 *
 * Bundled so the chapter can paint the moment its code runs. Every other
 * chapter fetches its text over the network, and that request cannot even
 * begin until the page's JavaScript has arrived. On the one chapter people
 * open most, that round trip was pure waiting.
 *
 * Genesis 1 only, and KJV only. Any other translation still goes to the
 * network, so switching to ASV or WEB behaves exactly as before.
 */

export type GenesisOneVerse = {
  verse: number;
  text: string;
};

export const GENESIS_ONE_KJV: GenesisOneVerse[] = [
...
/** True only for the one chapter and translation this file covers. */
export function isGenesisOneKjv(
  book: string | null | undefined,
  chapter: number | null | undefined,
  translation: string | null | undefined,
) {
  return (
    String(book || "").trim().toLowerCase() === "genesis" &&
    Number(chapter) === 1 &&
    String(translation || "").toLowerCase() === "kjv"
  );
}
