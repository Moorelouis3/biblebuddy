type GenesisOnePersonalSection = {
  chapter: number;
  startVerse: number;
  endVerse: number;
  reference: string;
...
export const GENESIS_ONE_RAW_NOTES = `# **🌅 Genesis 1:1–2**
...
type ParsedGenesisOneSection = GenesisOnePersonalSection & {
...
function parseReferenceParts(reference: string) {
  ...
  if (!match) {
    throw new Error(`Could not parse Genesis 1 reference: ${reference}`);
  }
...
function parseGenesisOneRawNotes(raw: string): ParsedGenesisOneSection[] {
  const lines = raw.replace(/\r\n/g, '\n').split('\n');
  const sections: ParsedGenesisOneSection[] = [];

  let currentSection: ParsedGenesisOneSection | null = null;
...
function buildGenesisOneDeepNotes(sections: ParsedGenesisOneSection[]) {
...
const parsedGenesisOneSections = parseGenesisOneRawNotes(GENESIS_ONE_RAW_NOTES);

export const GENESIS_ONE_PERSONAL_SECTIONS: GenesisOnePersonalSection[] = parsedGenesisOneSections.map(({ displayReference, ...section }) => section);

export const GENESIS_ONE_DEEP_NOTES = buildGenesisOneDeepNotes(parsedGenesisOneSections);
