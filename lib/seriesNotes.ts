...
const notesLoadCache = new Map<string, Promise<SeriesNotesPayload>>();
...
export async function loadSeriesNotesContent(
  ...
  const seriesKey = getSeriesKeyFromTitle(seriesTitle);
  const cacheKey = `${seriesKey ?? "fallback"}:${weekNumber}:${fallbackNotes ? "with-fallback" : "no-fallback"}`;

  const cached = notesLoadCache.get(cacheKey);
  if (cached) return cached;

  const loader = (async (): Promise<SeriesNotesPayload> => {
    if (seriesKey) {
  ...
  notesLoadCache.set(cacheKey, loader);
  return loader;
}
