...
function getExodusTwentyOneToThirtyTitleIcon(title: string) {
  ...
  if (/servant|maid|master|manservant|bondman|bondwoman|slave/i.test(title)) return "âš–ï¸";
  if (/smite|kill|death|life|eye|tooth|hand|foot|burning|wound|stripe|hurt/i.test(title)) return "âš–ï¸";
  if (/if a man|if men|come presumptuously|stealeth|curseth|strive|slay|wicked/i.test(title)) return "âš–ï¸";
  if (/ox|sheep|ass|beast|field|vineyard|fire|pit|restitution|restore/i.test(title)) return "ðŸ§¾";
  if (/owner|money|stuff|good|make good|pay|sum|loss|torn in pieces/i.test(title)) return "ðŸ§¾";
  if (/widow|fatherless|stranger|poor|enemy|neighbor|borrow|pledge/i.test(title)) return "ðŸ›¡ï¸";
  if (/false|witness|report|judge|judgment|cause|matter|controversy/i.test(title)) return "âš–ï¸";
  if (/sabbath|feast|firstfruits|bread|sacrifice|blood|covenant/i.test(title)) return "ðŸ™Œ";
  if (/ark|mercy seat|cherubim|table|candlestick|lamp|tabernacle|curtain|veil|court/i.test(title)) return "ðŸ•ï¸";
  if (/altar|burnt|offering|horn|brasen/i.test(title)) return "ðŸ©¸";
  if (/ephod|breastplate|robe|garment|mitre|stones|names/i.test(title)) return "ðŸ‘•";
  if (/priest|aaron|sons|holy|anoint|consecrate|wash|incense|oil/i.test(title)) return "ðŸ•¯ï¸";
  if (/gold|silver|brass|blue|purple|scarlet|linen|shittim/i.test(title)) return "ðŸŽ";
  if (/command|word|book|law|ordinance|statute|testimony/i.test(title)) return "ðŸ“œ";
  if (/angel|presence|lord|god|worship|serve/i.test(title)) return "ðŸ™Œ";
  if (/cubits|boards|rings|staves|sockets|hooks|loops|taches|branches|bowls/i.test(title)) return "ðŸ•ï¸";
  if (/seventh|free|redeemed|go out free|redeem/i.test(title)) return "ðŸ”“";
  if (/daughters|marriage|food|raiment|milk|kid|mother/i.test(title)) return "ðŸ ";
  if (/mount|cloud|come up|near|sapphire|forty days|forty nights|called unto moses/i.test(title)) return "â›°ï¸";
  if (/meet with thee|commune|faces|look one to another|willingly|heart/i.test(title)) return "ðŸ™Œ";
  if (/dishes|spoons|covers|tongs|snuffdishes|fashion|pattern|remnant|remaineth|covering/i.test(title)) return "ðŸ•ï¸";
  return "ðŸ”Ž";
}
...
function getExodusTwentyOneToThirtySectionIcon(section: PersonalExodusPhraseSectionInput) {
  ...
  if (/servant|injury|violence|justice|law|judgment/.test(text)) return "âš–ï¸";
  if (/theft|property|restitution|field|ox/.test(text)) return "ðŸ§¾";
  if (/widow|poor|stranger|neighbor|mercy/.test(text)) return "ðŸ›¡ï¸";
  if (/sabbath|feast|worship|covenant|blood/.test(text)) return "ðŸ™Œ";
  if (/ark|mercy seat/.test(text)) return "ðŸ“¦";
  if (/table|bread/.test(text)) return "ðŸž";
  if (/candlestick|lamp/.test(text)) return "ðŸ•¯ï¸";
  if (/tabernacle|curtain|veil|court/.test(text)) return "ðŸ•ï¸";
  if (/altar|sacrifice/.test(text)) return "ðŸ©¸";
  if (/garment|ephod|breastplate|priest/.test(text)) return "ðŸ‘•";
...
const RAW_EXODUS_21_30_PERSONAL_SECTIONS: PersonalExodusPhraseSectionInput[] = [
  {
    ...
    icon: "âš–ï¸",
    phrases: [
      phrase("ðŸ“œ These Are The Judgments", ["These are case laws.", "They apply covenant life to real situations.", "ðŸ“œ God's law", "ðŸ  Households", "âš–ï¸ Justice", "Exodus is showing that redemption must reshape ordinary society."]),
      phrase("â³ Six Years He Shall Serve", ["Servitude is limited.", "No Israelite servant is meant to be trapped forever under another Israelite.", "â³ Six years", "ðŸ”“ Seventh year release", "ðŸš« Not Pharaoh-style bondage", "A people rescued from slavery must not recreate Egypt among themselves."]),
      phrase("ðŸ”“ He Shall Go Out Free", ["Freedom is built into the law.", "The servant's release is not optional generosity.", "ðŸ”“ Free", "ðŸ“œ Commanded", "âš–ï¸ Protected", "God places limits on human power."]),
      phrase("ðŸ  If He Came In By Himself", ["The law pays attention to household situations.", "Freedom and responsibility are handled carefully.", "ðŸ  Marriage", "ðŸ‘¥ Family", "âš–ï¸ Case-by-case justice", "God's law enters complicated real life instead of staying abstract."]),
      phrase("â¤ï¸ I Love My Master", ["A servant may choose to stay because of love and household attachment.", "This is voluntary, not forced by Pharaoh-like control.", "â¤ï¸ Love", "ðŸ  Household", "ðŸ‘‚ Ear marked", "The passage distinguishes chosen belonging from endless oppression."]),
      phrase("ðŸ‘‚ Bore His Ear Through", ["The ear is marked as a sign of lifelong service.", "It is public and embodied.", "ðŸ‘‚ Ear", "ðŸšª Doorpost", "ðŸ“œ Witness", "The decision is not hidden; it becomes visible in the household community."]),
      phrase("ðŸ›¡ï¸ She Shall Not Go Out As The Menservants Do", ["The law gives specific protection to a vulnerable woman.", "Her situation is not treated carelessly.", "ðŸ›¡ï¸ Protection", "ðŸ‘© Vulnerable person", "âš–ï¸ Limits on male power", "God's law notices people who could easily be used and discarded."]),
      phrase("ðŸž Her Food, Her Raiment, And Her Duty Of Marriage", ["Basic provision and marital rights are protected.", "The man cannot neglect her while keeping control.", "ðŸž Food", "ðŸ‘— Clothing", "ðŸ¤ Covenant duty", "The law pushes responsibility into the details of daily care."]),
    ],
  },
  {
    chapter: 21,
    ...
    icon: "ðŸ©¸",
    phrases: [
      phrase("ðŸ©¸ He That Smiteth A Man", ["Human life is protected by serious law.", "Violence cannot be treated lightly.", "ðŸ©¸ Life", "âš–ï¸ Accountability", "ðŸš« Murder condemned", "The God who rescued Israel from violent oppression commands His people to value life."]),
      phrase("ðŸƒ I Will Appoint Thee A Place", ["The law distinguishes murder from accidental killing.", "Justice must be careful.", "ðŸƒ Refuge", "âš–ï¸ Discernment", "ðŸš« No mob justice", "God's law is not careless with either guilt or innocence."]),
      phrase("ðŸ‘ª Father Or Mother", ["Violence and dishonor toward parents are treated seriously.", "Family order matters in covenant life.", "ðŸ‘ª Parents", "ðŸ  Household", "âš–ï¸ Accountability", "The home is not outside God's justice."]),
      phrase("â›“ï¸ Stealeth A Man", ["Kidnapping is condemned.", "People must not be treated as property to steal and sell.", "â›“ï¸ Kidnapping", "ðŸš« Human trafficking", "âš–ï¸ Severe judgment", "This matters deeply in a book about liberation from bondage."]),
      phrase("âš–ï¸ Eye For Eye", ["The wording limits revenge.", "The response must fit the harm.", "âš–ï¸ Proportion", "ðŸš« No escalating vengeance", "ðŸ©¹ Injury addressed", "God's law restrains retaliation instead of unleashing it."]),
      phrase("ðŸ›¡ï¸ He Shall Let Him Go Free", ["If a servant is permanently injured, freedom is required.", "The master's power has consequences.", "ðŸ›¡ï¸ Protection", "ðŸ©¹ Injury", "ðŸ”“ Freedom", "The law refuses to let powerful people harm servants without cost."]),
      phrase("ðŸ‚ If An Ox Were Wont To Push", ["Known danger creates responsibility.", "Negligence matters before God.", "ðŸ‚ Dangerous ox", "âš ï¸ Owner warned", "âš–ï¸ Accountable", "Ignoring a known risk can become guilt."]),
      phrase("ðŸ§¾ He Shall Make It Good", ["Damage must be repaired.", "Justice includes restoration where possible.", "ðŸ§¾ Payment", "ðŸ‚ Loss", "ðŸ¤ Neighbor responsibility", "God cares about the real-world cost of harm."]),
    ],
  },
  {
    chapter: 22,
    ...
    icon: "ðŸ§¾",
    phrases: [
      phrase("ðŸ‚ He Shall Restore Five Oxen", ["Theft requires more than apology.", "The thief must repair the loss.", "ðŸ‚ Ox stolen", "ðŸ§¾ Restitution", "âš–ï¸ Costly justice", "Biblical justice cares about the person who was harmed."]),
      phrase("ðŸŒ™ Breaking Up", ["The law addresses a thief breaking in.", "Danger, fear, and defense are treated with care.", "ðŸŒ™ Night", "ðŸ  Home", "âš–ï¸ Context matters", "God's law does not flatten every case into one simple slogan."]),
      phrase("ðŸ”¥ If Fire Break Out", ["Even spreading damage creates responsibility.", "A person's actions can harm a neighbor's field.", "ðŸ”¥ Fire", "ðŸŒ¾ Field", "ðŸ§¾ Repayment", "Holiness includes caring about consequences."]),
      phrase("ðŸ¤² Delivered Unto His Neighbour To Keep", ["Entrusted property must be handled faithfully.", "Trust is sacred in community life.", "ðŸ¤² Entrusted goods", "ðŸ  Neighbor", "âš–ï¸ Accountability", "God cares about ordinary honesty."]),
      phrase("ðŸ™ The Oath Of The Lord", ["When facts are hidden, an oath before God matters.", "God is treated as witness.", "ðŸ™ Oath", "ðŸ‘€ God sees", "âš–ï¸ Dispute settled", "Truthfulness before God holds the community together."]),
      phrase("ðŸŽ Borrowed", ["Borrowing carries responsibility.", "Using another person's property is not casual.", "ðŸŽ Borrowed animal", "ðŸ§¾ Loss", "ðŸ¤ Neighbor care", "The law teaches people to treat another person's property with seriousness."]),
    ],
  },
  {
    chapter: 22,
    ...
    icon: "ðŸ›¡ï¸",
    phrases: [
      phrase("ðŸ’ Endow Her To Be His Wife", ["Sexual sin has social and covenant consequences.", "The woman must not be used and discarded.", "ðŸ’ Responsibility", "ðŸ‘© Protection", "âš–ï¸ Consequence", "God's law pushes against exploitation."]),
      phrase("ðŸš« Thou Shalt Not Suffer A Witch To Live", ["Israel must reject spiritual practices that turn from the LORD.", "Covenant worship cannot mix with occult rebellion.", "ðŸš« Forbidden practice", "ðŸ™ Loyalty to God", "ðŸ”¥ Holy seriousness", "The command guards Israel's worship from spiritual corruption."]),
      phrase("ðŸŒ Thou Shalt Neither Vex A Stranger", ["God protects the stranger because Israel knows what that pain feels like.", "Memory must become mercy.", "ðŸŒ Stranger", "ðŸœï¸ Egypt memory", "ðŸ›¡ï¸ Protection", "Former strangers must not become oppressors."]),
      phrase("ðŸ‘‚ I Will Surely Hear Their Cry", ["God hears widows and fatherless children.", "The vulnerable are not invisible.", "ðŸ‘‚ God hears", "ðŸ’” Affliction", "âš–ï¸ Judgment", "The God who heard Israel's cry now warns Israel that He hears other cries too."]),
      phrase("ðŸ’° Thou Shalt Not Be To Him As An Usurer", ["Need must not become an opportunity for exploitation.", "The poor are protected from predatory lending.", "ðŸ’° Money", "ðŸ¤² Mercy", "ðŸš« Crushing interest", "God's people must not profit from desperation."]),
      phrase("ðŸ§¥ Thou Shalt Deliver It Unto Him By That The Sun Goeth Down", ["A poor person's cloak may be needed for warmth at night.", "The law notices bodily need.", "ðŸ§¥ Cloak", "ðŸŒ™ Night", "ðŸ›¡ï¸ Compassion", "Justice is not cold paperwork; it cares whether someone can sleep."]),
      phrase("ðŸ‘‘ Thou Shalt Not Revile The Gods", ["The passage warns against contemptuous speech toward authority.", "Covenant people must not be reckless with words.", "ðŸ‘‘ Rulers", "ðŸ—£ï¸ Speech", "âš–ï¸ Restraint", "Holiness shapes how people speak even under authority structures."]),
      phrase("ðŸŒ¾ The First Of Thy Ripe Fruits", ["Israel must give God the first, not leftovers.", "Provision is received as gift.", "ðŸŒ¾ Firstfruits", "ðŸ‡ Plenty", "ðŸ™ Worship", "Giving trains the heart to remember who provides."]),
    ],
  },
  {
    chapter: 23,
    ...
    icon: "ðŸ—£ï¸",
    phrases: [
      phrase("ðŸ—£ï¸ Thou Shalt Not Raise A False Report", ["False words damage justice.", "God's people must not spread what is untrue.", "ðŸ—£ï¸ Report", "âš–ï¸ Court", "ðŸ¤ Neighbor", "Truth fits within covenant faithfulness."]),
      phrase("ðŸ‘¥ Thou Shalt Not Follow A Multitude To Do Evil", ["A crowd can make wrong feel normal.", "God warns Israel not to let numbers become conscience.", "ðŸ‘¥ Crowd", "ðŸš« Evil", "ðŸ§­ Moral courage", "Truth does not become false because many people prefer it."]),
      phrase("âš–ï¸ Neither Shalt Thou Countenance A Poor Man", ["Justice must not be bent by favoritism in any direction.", "The poor must be protected, but truth still matters.", "âš–ï¸ Impartiality", "ðŸ‘€ Truth", "ðŸš« Favoritism", "Biblical justice is compassion without corruption."]),
      phrase("ðŸ‚ Thine Enemy's Ox", ["Even an enemy's animal must be helped.", "Neighbor love reaches across personal hostility.", "ðŸ‚ Enemy's ox", "ðŸ¤² Return it", "ðŸ’” Restraint", "God's law trains people not to let hatred govern conduct."]),
      phrase("ðŸ’° Thou Shalt Take No Gift", ["Bribes blind judgment.", "Money can distort even wise people.", "ðŸ’° Gift", "ðŸ‘€ Blindness", "âš–ï¸ Twisted words", "Justice cannot be sold and still remain justice."]),
      phrase("ðŸŒ Ye Know The Heart Of A Stranger", ["Israel's memory of Egypt becomes a moral teacher.", "They know what it feels like to be vulnerable outsiders.", "ðŸŒ Stranger", "ðŸ’” Heart memory", "ðŸ›¡ï¸ Protection", "God turns remembered pain into commanded mercy."]),
    ],
  },
  {
    chapter: 23,
    ...
    icon: "ðŸ§­",
    phrases: [
      phrase("ðŸŒ¾ The Seventh Year Thou Shalt Let It Rest", ["Even the land receives rest.", "The poor and animals benefit from the release.", "ðŸŒ¾ Resting land", "ðŸ¤² Poor eat", "ðŸ¾ Beasts eat", "God's law resists endless extraction."]),
      phrase("ðŸ›‘ On The Seventh Day Thou Shalt Rest", ["Sabbath reaches servants, strangers, and animals.", "Rest is not only for the powerful.", "ðŸ›‘ Sabbath", "ðŸ‚ Ox", "ðŸŒ Stranger", "God builds mercy into time."]),
      phrase("ðŸŽ‰ Three Times Thou Shalt Keep A Feast", ["Israel's calendar becomes a teacher.", "Feasts keep rescue and provision in memory.", "ðŸŽ‰ Feast", "ðŸ“… Yearly rhythm", "ðŸ™ Worship", "Time itself is shaped around God."]),
      phrase("ðŸž Feast Of Unleavened Bread", ["This feast remembers the Exodus rescue.", "Israel must not forget the haste and mercy of deliverance.", "ðŸž Unleavened bread", "ðŸƒ Leaving Egypt", "ðŸ§  Memory", "Freedom gets placed on the calendar."]),
      phrase("ðŸ‘¼ I Send An Angel Before Thee", ["God promises guidance on the road ahead.", "Israel will not enter the future alone.", "ðŸ‘¼ Angel", "ðŸ§­ Way", "ðŸ›¡ï¸ Keeper", "The God who brought them out will lead them forward."]),
      phrase("âœ¨ My Name Is In Him", ["The angel carries God's authority in a mysterious way.", "This is no ordinary messenger.", "âœ¨ God's name", "ðŸ“£ Authority", "ðŸ§­ Guidance", "Israel must take God's leading seriously."]),
      phrase("ðŸŒ± By Little And Little", ["God says the land will not be taken all at once.", "Promise unfolds through process.", "ðŸŒ± Gradual growth", "â³ Patience", "ðŸžï¸ Land ahead", "Slow fulfillment is still faithfulness."]),
      phrase("ðŸš« Thou Shalt Make No Covenant With Them", ["Israel must not covenant with idolatry.", "Compromise would become a snare.", "ðŸš« No covenant", "ðŸª¤ Snare", "ðŸ™ Worship guarded", "The promised land must not become a place where Israel forgets the LORD."]),
    ],
  },
  {
    chapter: 24,
    ...
    icon: "ðŸ©¸",
    phrases: [
      phrase("â›°ï¸ Come Up Unto The Lord", ["God calls Moses, Aaron, Nadab, Abihu, and the elders near.", "Covenant includes ordered approach.", "â›°ï¸ Mountain", "ðŸ‘¥ Leaders", "ðŸ™ Worship afar", "Nearness to God is real, but still structured by His command."]),
      phrase("ðŸ“– Moses Told The People All The Words", ["The covenant is spoken to the people.", "They are not entering blind.", "ðŸ“– Words", "ðŸ‘‚ Hearing", "ðŸ¤ Response", "God's covenant people must hear God's covenant words."]),
      phrase("ðŸ™‹ All The Words Which The Lord Hath Said Will We Do", ["Israel answers with commitment.", "Their words are serious.", "ðŸ™‹ We will do", "ðŸ“œ God's commands", "ðŸ¤ Covenant promise", "This moment will make later disobedience even more weighty."]),
      phrase("ðŸª¨ Twelve Pillars", ["The twelve pillars represent the tribes of Israel.", "The whole people are represented before the altar.", "ðŸª¨ Pillars", "ðŸ‘¥ Twelve tribes", "ðŸ™ Covenant scene", "God is binding a people to Himself."]),
      phrase("ðŸ©¸ Half Of The Blood", ["Blood is placed on the altar and on the people.", "The covenant is sealed with life-blood.", "ðŸ©¸ Blood", "ðŸ”¥ Altar", "ðŸ‘¥ People", "Covenant with a holy God is not casual paperwork."]),
      phrase("ðŸ“– The Book Of The Covenant", ["The covenant words are written and read.", "Israel hears the terms again.", "ðŸ“– Book", "ðŸ‘‚ Public reading", "ðŸ¤ Covenant obedience", "God's people are shaped by spoken and written word."]),
      phrase("ðŸ©¸ Behold The Blood Of The Covenant", ["Moses names what the blood means.", "The people are marked into covenant relationship.", "ðŸ©¸ Covenant blood", "ðŸ‘¥ People sprinkled", "ðŸ“œ Words confirmed", "The relationship is sealed by God's appointed sign."]),
    ],
  },
  {
    chapter: 24,
    ...
    icon: "ðŸ½ï¸",
    phrases: [
      phrase("ðŸ‘€ They Saw The God Of Israel", ["The elders receive a real vision of God.", "The text is careful and reverent.", "ðŸ‘€ Saw", "ðŸ‘‘ God of Israel", "ðŸ’Ž Sapphire-like pavement", "The covenant brings astonishing nearness without making God common."]),
      phrase("ðŸ’Ž As It Were A Paved Work Of A Sapphire Stone", ["The description focuses under God's feet.", "The language reaches for beauty without pretending to contain God.", "ðŸ’Ž Sapphire", "ðŸŒŒ Clearness", "ðŸ‘‘ Majesty", "Some glory can be described, but not domesticated."]),
      phrase("ðŸ½ï¸ They Did Eat And Drink", ["The elders eat before God.", "Covenant nearness becomes fellowship.", "ðŸ½ï¸ Meal", "ðŸ‘¥ Elders", "ðŸ™ Before God", "The holy God who thundered at Sinai also allows covenant fellowship."]),
      phrase("ðŸª¨ Tables Of Stone", ["God calls Moses up to receive the tablets.", "The covenant words will be written by God.", "ðŸª¨ Stone", "ðŸ“œ Law", "ðŸ‘‘ Divine authority", "Israel's life will be ordered by God's own testimony."]),
      phrase("â˜ï¸ A Cloud Covered The Mount", ["The cloud marks God's presence on Sinai.", "Moses waits before entering deeper glory.", "â˜ï¸ Cloud", "â›°ï¸ Mountain", "â³ Six days", "God's presence is weighty and not rushed."]),
      phrase("ðŸ”¥ Like Devouring Fire", ["The glory of the LORD appears like devouring fire.", "Israel sees holiness from below.", "ðŸ”¥ Fire", "âœ¨ Glory", "ðŸ˜¨ Awe", "The God of covenant fellowship is still the holy God."]),
      phrase("â³ Forty Days And Forty Nights", ["Moses remains on the mountain for forty days and nights.", "The people must wait.", "â³ Forty days", "â›°ï¸ Mountain", "ðŸ“œ Instruction", "This waiting period will become a test in Exodus 32."]),
    ],
  },
  {
    chapter: 25,
    ...
    icon: "ðŸ“¦",
    phrases: [
      phrase("ðŸ¤² Of Every Man That Giveth It Willingly", ["The tabernacle offering begins with willing hearts.", "God does not present holy construction as forced extraction.", "ðŸ¤² Willing gift", "ðŸ’› Heart", "ðŸ•ï¸ Dwelling", "The rescued people are invited to bring materials for God's sanctuary."]),
      phrase("ðŸ•ï¸ Let Them Make Me A Sanctuary", ["God wants to dwell among His people.", "This is the heart of the tabernacle instructions.", "ðŸ•ï¸ Sanctuary", "ðŸ‘¥ Among them", "âœ¨ God's presence", "Exodus moves from rescue to dwelling."]),
      phrase("ðŸ“ According To All That I Shew Thee", ["The tabernacle is built by God's pattern.", "Worship is received, not invented.", "ðŸ“ Pattern", "ðŸ‘€ Shown to Moses", "ðŸ› ï¸ Obedient making", "Holy space must be shaped by God's word."]),
      phrase("ðŸ“¦ An Ark Of Shittim Wood", ["The ark is the central covenant chest.", "It will hold the testimony.", "ðŸ“¦ Ark", "ðŸªµ Wood", "ðŸ“œ Covenant witness", "The furniture is not random decoration; it carries covenant meaning."]),
      phrase("ðŸ‘‘ A Crown Of Gold Round About", ["The ark is overlaid with gold and crowned.", "Its beauty matches its holy purpose.", "ðŸ‘‘ Gold", "âœ¨ Holy beauty", "ðŸ“¦ Ark", "God's dwelling is marked by glory, not carelessness."]),
      phrase("ðŸª½ Cherubims", ["Cherubim are placed over the mercy seat.", "They echo guarded holy presence.", "ðŸª½ Cherubim", "ðŸ©¸ Mercy seat", "ðŸ”¥ Holy space", "Access to God is merciful, but never casual."]),
      phrase("ðŸ©¸ Mercy Seat", ["The mercy seat is the place where God will meet with Moses.", "It is connected to atonement and covenant presence.", "ðŸ©¸ Mercy", "ðŸ“¦ Ark", "ðŸ—£ï¸ God speaks", "God's throne among Israel is a place of holy mercy."]),
      phrase("ðŸ—£ï¸ There I Will Meet With Thee", ["God promises to meet and speak from above the mercy seat.", "The tabernacle is about relationship, not furniture alone.", "ðŸ—£ï¸ Speech", "ðŸ¤ Meeting", "ðŸ•ï¸ Presence", "God provides a place where His word comes to His people."]),
    ],
  },
  {
    chapter: 25,
    ...
    icon: "ðŸ•¯ï¸",
    phrases: [
      phrase("ðŸž A Table Of Shittim Wood", ["The table belongs in holy space.", "It will hold bread before the LORD.", "ðŸž Table", "ðŸ•ï¸ Holy place", "ðŸ¤² Provision remembered", "God's presence includes fellowship and provision."]),
      phrase("ðŸž Shewbread", ["The bread is placed before God continually.", "Israel's tribes are represented in God's presence.", "ðŸž Bread", "ðŸ‘¥ People remembered", "â³ Continual", "The table teaches ongoing dependence and fellowship."]),
      phrase("ðŸ•¯ï¸ A Candlestick Of Pure Gold", ["The lampstand brings light into holy space.", "It is carefully made and beautiful.", "ðŸ•¯ï¸ Lampstand", "âœ¨ Gold", "ðŸ•ï¸ Sanctuary light", "Nearness to God is filled with ordered light."]),
      phrase("ðŸŒ¸ Bowls Made Like Unto Almonds", ["The lampstand is shaped with plant-like beauty.", "Holy design includes artistry.", "ðŸŒ¸ Almond blossoms", "ðŸŽ¨ Beauty", "ðŸ•¯ï¸ Light", "God values crafted beauty in worship."]),
      phrase("ðŸ”¥ Seven Lamps", ["The seven lamps give light over against the lampstand.", "The holy place is not dark.", "ðŸ”¥ Lamps", "7ï¸âƒ£ Fullness", "ðŸ’¡ Illumination", "God provides light for priestly service."]),
      phrase("ðŸ‘€ Look That Thou Make Them After Their Pattern", ["Moses must follow the pattern shown on the mountain.", "The details matter because they are God's details.", "ðŸ‘€ Look", "ðŸ“ Pattern", "ðŸ› ï¸ Obedience", "Worship is not a place for careless improvisation."]),
    ],
  },
  {
    chapter: 26,
    ...
    icon: "ðŸ§µ",
    phrases: [
      phrase("ðŸ§µ Fine Twined Linen", ["The tabernacle begins with carefully made curtains.", "Holy space is crafted with skill.", "ðŸ§µ Linen", "ðŸŽ¨ Workmanship", "ðŸ•ï¸ Dwelling", "God's dwelling among Israel is not thrown together."]),
      phrase("ðŸª½ Cherubims Of Cunning Work", ["Cherubim are woven into the curtains.", "The imagery marks the space as holy.", "ðŸª½ Cherubim", "ðŸ§µ Woven beauty", "ðŸ”¥ Guarded presence", "The tabernacle carries visual reminders of heavenly holiness."]),
      phrase("ðŸ”— Coupled Together", ["The curtains are joined into one tabernacle.", "Many pieces become one dwelling.", "ðŸ”— Loops and clasps", "ðŸ§µ Joined curtains", "ðŸ•ï¸ One tent", "The details teach ordered unity."]),
      phrase("ðŸ Curtains Of Goats' Hair", ["Outer coverings protect the holy space.", "The dwelling has layers.", "ðŸ Goats' hair", "ðŸ•ï¸ Covering", "ðŸ›¡ï¸ Protection", "God's instructions include both beauty and durability."]),
      phrase("ðŸ›¡ï¸ Rams' Skins Dyed Red", ["More coverings are placed over the tent.", "The holy dwelling is protected from outside exposure.", "ðŸ›¡ï¸ Covering", "ðŸ”´ Red skins", "ðŸ•ï¸ Shelter", "The sanctuary is a guarded place of nearness."]),
    ],
  },
  {
    chapter: 26,
    ...
    icon: "ðŸšª",
    phrases: [
      phrase("ðŸªµ Boards For The Tabernacle", ["The tabernacle has a stable frame.", "God gives structure to the dwelling.", "ðŸªµ Boards", "ðŸ“ Measurements", "ðŸ•ï¸ Stability", "Holy space has order and form."]),
      phrase("ðŸ¥ˆ Sockets Of Silver", ["The boards rest in silver sockets.", "The dwelling is carefully grounded.", "ðŸ¥ˆ Silver", "ðŸªµ Boards", "ðŸ“ Fitted design", "The details show strength and beauty working together."]),
      phrase("ðŸ”© Bars Of Shittim Wood", ["Bars hold the frames together.", "The tabernacle is joined and strengthened.", "ðŸ”© Bars", "ðŸªµ Frames", "ðŸ”— Unity", "God's dwelling is built with careful connection."]),
      phrase("ðŸšª A Vail", ["The veil separates the holy place from the most holy.", "Access is real, but guarded.", "ðŸšª Veil", "ðŸ“¦ Ark behind it", "ðŸ”¥ Holiness", "God is near, but sinful people cannot rush in however they want."]),
      phrase("ðŸ“¦ Within The Vail", ["The ark belongs inside the veil.", "The most holy place centers on God's covenant presence.", "ðŸ“¦ Ark", "ðŸšª Veil", "ðŸ©¸ Mercy seat", "The heart of the tabernacle is guarded mercy."]),
      phrase("ðŸž The Table Without The Vail", ["The table is outside the veil in the holy place.", "Each piece has its assigned place.", "ðŸž Table", "ðŸ•¯ï¸ Lampstand", "ðŸ“ Ordered placement", "Worship space is arranged by God's wisdom."]),
      phrase("ðŸŒˆ An Hanging For The Door", ["The entrance has a crafted hanging.", "The way into holy space is marked and ordered.", "ðŸŒˆ Blue, purple, scarlet", "ðŸšª Door", "ðŸŽ¨ Needlework", "Approach to God is beautiful, but still defined by God."]),
    ],
  },
  {
    chapter: 27,
    ...
    icon: "ðŸ”¥",
    phrases: [
      phrase("ðŸ”¥ An Altar Of Shittim Wood", ["The altar stands in the courtyard.", "Sacrifice belongs at the approach to God.", "ðŸ”¥ Altar", "ðŸªµ Wood", "ðŸ©¸ Sacrifice", "The path toward God's dwelling passes through atonement."]),
      phrase("ðŸ¦¬ Horns Of It", ["The altar has horns at its corners.", "They are part of its strength and ritual design.", "ðŸ¦¬ Horns", "ðŸ”¥ Altar", "ðŸ“ Holy pattern", "Even the altar's shape is given by God."]),
      phrase("ðŸ§º Pans, Shovels, Basons", ["The tools for altar service are named.", "Holy worship includes practical work.", "ðŸ§º Tools", "ðŸ”¥ Ashes", "ðŸ©¸ Service", "The details remind us that worship involves real labor, not vague feelings."]),
      phrase("ðŸ•ï¸ The Court Of The Tabernacle", ["The court creates a defined space around the dwelling.", "There is an outside and an inside.", "ðŸ•ï¸ Court", "ðŸš§ Boundaries", "ðŸ™ Approach", "God provides access, but ordered access."]),
      phrase("ðŸ§µ Fine Twined Linen", ["The court hangings mark off holy space.", "The boundary itself has beauty.", "ðŸ§µ Linen", "ðŸ“ Measurements", "ðŸ•ï¸ Sacred boundary", "Holiness is made visible through separation and order."]),
      phrase("ðŸ¥‰ Pins Of The Court", ["Even the pins are mentioned.", "Small parts matter in the structure God commands.", "ðŸ¥‰ Pins", "ðŸ•ï¸ Stability", "ðŸ› ï¸ Faithful details", "Nothing is too small when it serves God's dwelling."]),
    ],
  },
  {
    chapter: 27,
    ...
    icon: "ðŸ•¯ï¸",
    phrases: [
      phrase("ðŸ«’ Pure Oil Olive", ["The lamp requires pure olive oil.", "God specifies the fuel for holy light.", "ðŸ«’ Oil", "ðŸ•¯ï¸ Lamp", "âœ¨ Purity", "Holy service is sustained by what God commands."]),
      phrase("ðŸ•¯ï¸ To Cause The Lamp To Burn Always", ["The light is to be kept burning continually.", "The holy place is not left dark.", "ðŸ•¯ï¸ Light", "â³ Continual", "ðŸ™ Service", "God's dwelling is marked by ongoing light."]),
      phrase("ðŸ‘¥ Aaron And His Sons", ["The priests tend the lamp.", "Holy light requires faithful service.", "ðŸ‘¥ Priests", "ðŸ•¯ï¸ Tending", "ðŸ“… Evening to morning", "God appoints people to maintain what serves His presence."]),
      phrase("ðŸ“œ A Statute For Ever", ["This is not a one-time instruction.", "It becomes part of Israel's ongoing worship life.", "ðŸ“œ Statute", "ðŸ‘¥ Generations", "ðŸ•¯ï¸ Continued light", "God's worship forms rhythms across generations."]),
    ],
  },
  {
    chapter: 28,
    ...
    icon: "ðŸ‘•",
    phrases: [
      phrase("ðŸ‘¥ Aaron Thy Brother", ["Aaron and his sons are set apart for priestly service.", "The priesthood is appointed, not self-invented.", "ðŸ‘¥ Aaron", "ðŸ‘• Sons", "ðŸ™ Minister unto God", "Holy service begins with God's calling."]),
      phrase("âœ¨ Holy Garments", ["The priestly garments are called holy.", "Clothing becomes part of consecrated service.", "âœ¨ Holy", "ðŸ‘• Garments", "ðŸ™ Priesthood", "Nearness to God shapes even what the priest wears."]),
      phrase("ðŸ‘‘ For Glory And For Beauty", ["The garments are meant to carry weight and beauty.", "Worship is not careless or ugly in God's design.", "ðŸ‘‘ Glory", "ðŸŽ¨ Beauty", "ðŸ™ Service", "God's holiness is reflected through ordered beauty."]),
      phrase("ðŸ§  Wise Hearted", ["Skilled workers make the garments.", "Their wisdom is given by God.", "ðŸ§  Wisdom", "ðŸ› ï¸ Skill", "ðŸ‘• Holy garments", "Craftsmanship can be Spirit-enabled service."]),
      phrase("ðŸ’Ž Two Onyx Stones", ["The stones carry the names of Israel's sons.", "The priest bears the people before God.", "ðŸ’Ž Stones", "ðŸ‘¥ Names", "ðŸ™ Representation", "Priestly ministry is not about private status; it carries the people."]),
      phrase("ðŸ’ª Upon His Shoulders", ["The names are carried on the priest's shoulders.", "Shoulders picture bearing weight.", "ðŸ’ª Shoulders", "ðŸ‘¥ Israel's names", "ðŸ™ Before the LORD", "The priest's service includes carrying the covenant people."]),
    ],
  },
  {
    chapter: 28,
    ...
    icon: "ðŸ’Ž",
    phrases: [
      phrase("ðŸ’Ž Breastplate Of Judgment", ["The breastplate is connected with priestly judgment and discernment.", "It is beautiful and serious.", "ðŸ’Ž Breastplate", "âš–ï¸ Judgment", "ðŸ™ Priest before God", "The priest represents Israel in matters requiring God's direction."]),
      phrase("ðŸ‘¥ According To The Twelve Tribes", ["The stones represent Israel's tribes.", "Each tribe is remembered before God.", "ðŸ‘¥ Twelve tribes", "ðŸ’Ž Stones", "â¤ï¸ Over the heart", "God's people are carried personally, not anonymously."]),
      phrase("â¤ï¸ Upon Aaron's Heart", ["Aaron bears the names over his heart.", "Representation includes affection and burden.", "â¤ï¸ Heart", "ðŸ‘¥ Names", "ðŸ™ Before the LORD", "The priest carries the people close, not merely officially."]),
      phrase("âš–ï¸ Urim And Thummim", ["These are connected with discerning judgment before the LORD.", "Israel needs God's guidance.", "âš–ï¸ Judgment", "ðŸ™ Before God", "ðŸ§­ Direction", "Covenant leadership depends on God's wisdom, not only human calculation."]),
      phrase("ðŸ”” Bells Of Gold", ["The robe has bells around the hem.", "The priest's movement is heard in holy service.", "ðŸ”” Bells", "ðŸ‘• Robe", "ðŸ”¥ Holy place", "The detail teaches the seriousness of entering God's presence."]),
      phrase("âœ¨ Holiness To The Lord", ["The gold plate on Aaron's forehead names the priestly calling.", "His service is marked as set apart for God.", "âœ¨ Holiness", "ðŸ‘‘ Crown", "ðŸ™ Service", "Ministry is holy identity, not personal platform."]),
      phrase("ðŸ©³ Linen Breeches", ["Even undergarments are commanded.", "God protects modesty in holy service.", "ðŸ©³ Linen", "ðŸ›¡ï¸ Covering", "ðŸ”¥ Near altar", "Nothing about priestly service is outside God's concern."]),
      phrase("âš ï¸ That They Bear Not Iniquity", ["Priests must serve as God commands.", "Holy service mishandled brings guilt.", "âš ï¸ Warning", "ðŸ™ Service", "ðŸ”¥ Holiness", "Nearness to God is gift and responsibility."]),
    ],
  },
  {
    chapter: 29,
    ...
    icon: "ðŸ©¸",
    phrases: [
      phrase("ðŸ§¼ Wash Them With Water", ["Priestly service begins with washing.", "The priests do not approach God by self-confidence.", "ðŸ§¼ Washing", "ðŸ‘¥ Priests", "ðŸ™ Preparation", "Cleansing comes before service."]),
      phrase("ðŸ‘• Put Upon Aaron The Coat", ["Aaron is clothed for his role.", "The garments mark him for holy service.", "ðŸ‘• Clothing", "ðŸ‘‘ Priesthood", "âœ¨ Glory and beauty", "The priest is dressed according to God's command, not personal taste."]),
      phrase("ðŸ«’ Anointing Oil", ["Oil is poured on Aaron's head.", "He is set apart for priestly ministry.", "ðŸ«’ Oil", "ðŸ‘‘ Head", "ðŸ™ Consecration", "God marks the priest as belonging to holy service."]),
      phrase("ðŸ‚ A Bullock For A Sin Offering", ["The priests need a sin offering.", "Their role does not make them sinless.", "ðŸ‚ Bull", "ðŸ©¸ Sin offering", "ðŸ™ Atonement", "The mediator must first receive cleansing."]),
      phrase("âœ‹ Put Their Hands Upon The Head", ["The priests identify with the offering.", "The sacrifice is not detached from them.", "âœ‹ Hands", "ðŸ‚ Offering", "ðŸ©¸ Identification", "Atonement involves represented life."]),
      phrase("ðŸ”¥ Burn The Whole Ram", ["The burnt offering rises completely to God.", "Consecration includes whole surrender.", "ðŸ”¥ Burnt offering", "ðŸ Ram", "ðŸ™ Given to God", "Priestly service is not half-owned by the priest."]),
      phrase("ðŸ‘‚ Tip Of The Right Ear", ["Blood marks ear, thumb, and toe.", "The whole life is consecrated.", "ðŸ‘‚ Hearing", "ðŸ‘ Serving", "ðŸ¦¶ Walking", "Priests are marked to hear, act, and walk before God."]),
    ],
  },
  {
    chapter: 29,
    ...
    icon: "ðŸ”¥",
    phrases: [
      phrase("ðŸ¤² Wave Offering", ["Parts of the sacrifice are waved before the LORD.", "The priestly portion is first presented to God.", "ðŸ¤² Waved", "ðŸ”¥ Offered", "ðŸ™ Before the LORD", "Even what the priest receives is acknowledged as God's."]),
      phrase("â³ Seven Days", ["Consecration lasts seven days.", "The process is complete and ordered.", "â³ Seven days", "ðŸ™ Ordination", "ðŸ”¥ Altar sanctified", "Holy service is not rushed."]),
      phrase("ðŸ”¥ Continually", ["Daily offerings are made morning and evening.", "Worship is sustained over time.", "ðŸ”¥ Morning lamb", "ðŸŒ™ Evening lamb", "â³ Continual", "Israel's life near God is rhythmic, not occasional."]),
      phrase("ðŸž Flour Mingled With Oil", ["The daily offering includes grain and drink elements.", "Worship involves provision given back to God.", "ðŸž Flour", "ðŸ«’ Oil", "ðŸ· Drink offering", "Daily food symbols become daily devotion."]),
      phrase("ðŸ¤ There I Will Meet With The Children Of Israel", ["God promises meeting at the tabernacle.", "The goal is relationship.", "ðŸ¤ Meeting", "ðŸ‘¥ Israel", "âœ¨ Glory", "The tabernacle is about God's presence among His people."]),
      phrase("ðŸ•ï¸ I Will Dwell Among The Children Of Israel", ["This is one of the great lines of Exodus.", "God rescued Israel so He could dwell with them.", "ðŸ•ï¸ Dwell", "ðŸ‘¥ Among them", "ðŸ‘‘ Their God", "Redemption aims at presence."]),
      phrase("ðŸ§  They Shall Know That I Am The Lord", ["Deliverance and dwelling reveal God's identity.", "The people are meant to know Him.", "ðŸ§  Know", "ðŸ”“ Brought out", "ðŸ•ï¸ Dwelling God", "The Exodus is not only escape from Egypt; it is knowing the LORD."]),
    ],
  },
  {
    chapter: 30,
    ...
    icon: "ðŸŒ«ï¸",
    phrases: [
      phrase("ðŸŒ«ï¸ An Altar To Burn Incense", ["The incense altar stands inside holy service.", "Prayer-like fragrance rises before God.", "ðŸŒ«ï¸ Incense", "ðŸ”¥ Altar", "ðŸ™ Near presence", "The tabernacle includes not only sacrifice, but fragrant worship."]),
      phrase("ðŸ‘‘ A Crown Of Gold Round About", ["The incense altar has a gold crown.", "Its design is beautiful and holy.", "ðŸ‘‘ Gold crown", "ðŸŒ«ï¸ Incense", "ðŸ•ï¸ Holy place", "God gives beauty to the place of continual worship."]),
      phrase("ðŸŒ… Every Morning", ["Aaron burns incense every morning.", "Holy service has daily rhythm.", "ðŸŒ… Morning", "ðŸŒ«ï¸ Incense", "ðŸ•¯ï¸ Lamps dressed", "Each day begins with tended light and rising fragrance."]),
      phrase("ðŸŒ™ At Even", ["Incense is also burned in the evening.", "The day closes with holy service.", "ðŸŒ™ Evening", "ðŸ•¯ï¸ Lamps lit", "ðŸŒ«ï¸ Incense", "Worship surrounds the daily life of Israel."]),
      phrase("ðŸš« Strange Incense", ["God forbids unauthorized incense.", "Holy things must not be invented casually.", "ðŸš« Strange incense", "ðŸ“œ God's command", "ðŸ”¥ Holy boundary", "Worship must be shaped by God's word."]),
      phrase("ðŸ©¸ Once In A Year", ["Atonement is made on the horns of the incense altar once a year.", "Even the altar of fragrance needs blood applied.", "ðŸ©¸ Atonement", "ðŸŒ«ï¸ Incense altar", "ðŸ“… Yearly", "Access and worship still depend on cleansing."]),
    ],
  },
  {
    chapter: 30,
    ...
    icon: "ðŸ’§",
    phrases: [
      phrase("ðŸ§® When Thou Takest The Sum", ["Counting the people requires atonement money.", "Israel's lives belong to God.", "ðŸ§® Numbering", "ðŸª™ Ransom", "ðŸ™ Before the LORD", "Even administration is brought under worship."]),
      phrase("ðŸª™ A Ransom For His Soul", ["Each person gives ransom money.", "The language teaches that life must be covered before God.", "ðŸª™ Ransom", "ðŸ‘¤ Soul", "âš–ï¸ Atonement", "Israel is counted as a redeemed people."]),
      phrase("âš–ï¸ The Rich Shall Not Give More", ["The ransom amount is the same for rich and poor.", "Every life stands equal in this act.", "âš–ï¸ Same amount", "ðŸ’° Rich", "ðŸ¤² Poor", "Atonement is not priced by status."]),
      phrase("ðŸ•ï¸ Service Of The Tabernacle", ["The atonement money supports tabernacle service.", "Redemption and worship are connected.", "ðŸ•ï¸ Tabernacle", "ðŸª™ Money", "ðŸ“œ Memorial", "The people's ransom becomes part of the dwelling-place work."]),
      phrase("ðŸ’§ A Laver Of Brass", ["The laver provides water for priestly washing.", "Service near God requires cleansing.", "ðŸ’§ Laver", "ðŸ‘ Hands", "ðŸ¦¶ Feet", "Priests must be washed before approaching the altar or tent."]),
      phrase("âš ï¸ That They Die Not", ["The warning is repeated.", "Holy service without cleansing is dangerous.", "âš ï¸ Warning", "ðŸ’§ Washing", "ðŸ”¥ Altar", "God's nearness is mercy, but it is never casual."]),
    ],
  },
  {
    chapter: 30,
    ...
    icon: "ðŸ«’",
    phrases: [
      phrase("ðŸ«’ Principal Spices", ["The anointing oil is made from costly spices.", "Holy service is marked by richness and care.", "ðŸ«’ Oil", "ðŸŒ¿ Spices", "âœ¨ Set apart", "Holy things are not treated as common."]),
      phrase("âœ¨ An Holy Anointing Oil", ["The oil consecrates the tabernacle, furniture, and priests.", "It marks them as set apart for God.", "âœ¨ Holy oil", "ðŸ•ï¸ Tabernacle", "ðŸ‘¥ Priests", "Anointing sets apart people and objects for holy service."]),
      phrase("ðŸš« Upon Man's Flesh Shall It Not Be Poured", ["The oil must not be used casually.", "It is not ordinary perfume.", "ðŸš« Not common", "âœ¨ Holy", "ðŸ“œ Guarded use", "What God sets apart must not be turned into personal luxury."]),
      phrase("ðŸŒ«ï¸ A Perfume", ["The incense is carefully blended.", "Scent becomes part of holy worship.", "ðŸŒ«ï¸ Incense", "ðŸŒ¿ Spices", "ðŸ™ Before testimony", "God's worship engages the senses, but under His command."]),
      phrase("âš–ï¸ Tempered Together, Pure And Holy", ["The incense is balanced, pure, and holy.", "It is crafted with skill.", "âš–ï¸ Blended", "âœ¨ Pure", "ðŸŒ«ï¸ Holy fragrance", "Beauty in worship is ordered and consecrated."]),
      phrase("ðŸš« Ye Shall Not Make To Yourselves", ["The incense recipe must not be copied for personal use.", "Holy fragrance is reserved for God's service.", "ðŸš« No imitation", "ðŸ™ Reserved for God", "ðŸ”¥ Holy boundary", "God guards the difference between holy and common."]),
    ],
  },
];
...
function makeBeginnerExodusPhrase(title: string, section: PersonalExodusPhraseSectionInput, focus: string): [string, string] {
  return phrase(title, [
    ...
    "âš–ï¸ What is fair",
    "ðŸ›¡ï¸ Who is protected",
    "ðŸ§¾ What is required",
    "ðŸ™Œ How worship stays holy",
...
function ensureBeginnerExodusPhraseDepth(section: PersonalExodusPhraseSectionInput): PersonalExodusPhraseSectionInput {
  const additions: Array<[string, string]> = [
    makeBeginnerExodusPhrase("ðŸ§­ What Is Happening Here?", section, "The wording helps the reader locate the law or covenant scene: who is being protected, what wrong is being limited, and what kind of people Israel must become."),
    makeBeginnerExodusPhrase("ðŸ”Ž Why This Detail Matters", section, "This detail matters because Exodus moves from rescue to formation; God is shaping daily life, not only dramatic worship moments."),
    makeBeginnerExodusPhrase("ðŸ§  Beginner Connection", section, "A new Bible reader may not know why these laws are here, but they show that God's salvation changes households, courts, money, animals, work, rest, and worship."),
    makeBeginnerExodusPhrase("ðŸ§µ Watch The Pattern", section, "Watch the pattern: God protects vulnerable people, limits retaliation, commands honest worship, and makes covenant life visible."),
    makeBeginnerExodusPhrase("â¤ï¸ What This Shows About People", section, "This scene shows people need boundaries because power, anger, greed, negligence, and false worship can damage a community."),
    makeBeginnerExodusPhrase("ðŸ™Œ What This Shows About God", section, "This scene shows the LORD cares about justice in ordinary life, not only songs, sacrifices, and mountain experiences."),
  ];
...
function makeDay27PhraseCard(section: PersonalExodusPhraseSectionInput, title: string): [string, string] {
  return [`ðŸ“Œ ${title}`, explainDay27Phrase(section, title)];
}
...
function escapePhraseForRegex(value: string): string {
  return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}

function stripTitleFromOpening(title: string, line: string): string {
  const escaped = escapePhraseForRegex(title);
  const patterns = [
    new RegExp(`^${escaped} means\\s+`, "i"),
    new RegExp(`^${escaped} are\\s+`, "i"),
    new RegExp(`^${escaped} names\\s+`, "i"),
    new RegExp(`^${escaped} gives\\s+`, "i"),
    new RegExp(`^${escaped} teaches\\s+`, "i"),
    new RegExp(`^${escaped} explains\\s+`, "i"),
    new RegExp(`^${escaped} warns\\s+`, "i"),
    new RegExp(`^${escaped} closes\\s+`, "i"),
    new RegExp(`^${escaped} prepares\\s+`, "i"),
    new RegExp(`^${escaped} begins\\s+`, "i"),
    new RegExp(`^${escaped} connects\\s+`, "i"),
    new RegExp(`^${escaped} protects\\s+`, "i"),
    new RegExp(`^${escaped} places\\s+`, "i"),
    new RegExp(`^${escaped} repeats\\s+`, "i"),
    new RegExp(`^${escaped} introduces\\s+`, "i"),
    new RegExp(`^${escaped} treats\\s+`, "i"),
    new RegExp(`^${escaped} is\\s+`, "i"),
    new RegExp(`^${escaped} celebrates\\s+`, "i"),
    new RegExp(`^${escaped} comes\\s+`, "i"),
    new RegExp(`^${escaped} describes\\s+`, "i"),
    new RegExp(`^${escaped} marks\\s+`, "i"),
    new RegExp(`^${escaped} runs\\s+`, "i"),
  ];
...
function finalizeExodusPhraseExplanation(
  title: string,
  lines: string[],
  ...
  lines[0] = keyedOverride ?? overrides?.[title] ?? stripTitleFromOpening(title, lines[0]);
  return note(lines.slice(0, 8));
}

const DAY_27_FIRST_LINE_OVERRIDES: Record<string, string> = {
  "After The Manner Of Daughters": "A daughter in this setting was to receive food, clothing, and marriage rights, not be discarded at a man's convenience.",
  "Life For Life": "This is the strongest form of measured justice: a life taken could not be treated like a minor loss.",
  "Eye For Eye": "Justice for injury had to match the wound instead of growing into personal revenge.",
  "Tooth For Tooth": "A damaged tooth stands for real bodily injury that could not be dismissed or exaggerated.",
  "Hand For Hand": "Damage to a person's ability to work or act had to be judged with equal seriousness.",
  "Foot For Foot": "Injury that affects walking or daily life had to be answered with fair proportion.",
  "The Feast Of Harvest": "This feast thanked the LORD for the first gathered produce of the year.",
  "The Feast Of Ingathering": "This feast marked the final gathering of produce at the end of the agricultural year.",
  "To Keep Thee In The Way": "The angel is sent to guard Israel on the road and bring them safely where God promised.",
  "Nor Be Barren": "God promises fruitfulness instead of the loss and emptiness people feared in the land.",
  "Nor With Their Gods": "Israel must not enter loyalty agreements with the false gods of the land.",
  "They Shall Not Dwell In Thy Land": "The nations must not remain in a way that would keep teaching Israel idolatry.",
  "Come Up Unto The LORD": "This is a summons to draw near on Sinai under God's terms, not a casual invitation.",
  "All The Words Of The LORD": "Moses is repeating the covenant commands exactly as God gave them.",
  "Twelve Pillars": "The twelve pillars stand for the twelve tribes as the covenant is being confirmed.",
  "They Saw The God Of Israel": "The leaders are given a real vision of God's manifested glory without collapsing His holiness into an idol.",
  "The Glory Of The LORD Abode": "God's glory settled on Sinai as a lasting sign of His holy presence.",
  "A Devouring Fire": "From below, God's glory looked like consuming fire on the top of the mountain.",
};

const DAY_28_FIRST_LINE_OVERRIDES: Record<string, string> = {
  "Exodus 25:31-40|Oil For The Light": "The lampstand needed oil so light would continue inside the holy place.",
  "Exodus 25:31-40|Onyx Stones": "Onyx stones were costly materials later used in the high priest's garments.",
  "Exodus 25:9-14|According To All That I Shew Thee": "Moses must build from the pattern God reveals, not from personal preference.",
  "Exodus 25:9-14|Shittim Wood": "Shittim wood was the durable wood used to build the ark itself.",
  "Exodus 25:15-22|Shittim Wood": "Shittim wood forms the ark beneath the gold covering, giving the holy chest real structure.",
  "Exodus 25:23-28|Shittim Wood": "Shittim wood is used again for the table that will stand in the holy place.",
  "Exodus 26:15-20|Shittim Wood Standing Up": "These upright boards form the frame of the tabernacle walls.",
  "Exodus 26:21-26|Bars Of Shittim Wood": "The bars of shittim wood hold the tabernacle boards together across the sides.",
  "Exodus 27:1-6|Overlay It With Brass": "Brass covers the altar because this outer court furniture is built for sacrificial fire.",
  "Exodus 27:7-8|The Altar May Be Borne": "The altar is fitted to be carried when Israel travels.",
  "Exodus 27:7-8|So Shall They Make It": "The builders must copy the mountain pattern exactly when making the altar.",
  "Exodus 27:9-14|Fine Twined Linen": "Fine twined linen forms the courtyard hangings that mark off holy space.",
  "Exodus 27:9-14|Twenty Pillars": "Twenty pillars support the long side of the courtyard boundary.",
  "Exodus 27:9-14|The Gate Of The Court": "The gate is the appointed entrance into the courtyard around the tabernacle.",
  "Exodus 27:15-20|Filleted With Silver": "Silver fillets are the connecting bands that finish and join the courtyard pillars.",
  "Exodus 27:15-20|Sockets Of Brass": "Brass sockets are the heavy bases that hold the courtyard pillars in place.",
  "Exodus 27:21-21|Before The LORD": "The lamp burns in God's presence, not merely for human visibility.",
  "Exodus 27:21-21|Aaron And His Sons": "Aaron and his sons are responsible for tending the lamp as part of priestly service.",
  "Exodus 27:21-21|From Evening To Morning": "The lamp was to be kept burning through the night until morning.",
  "Exodus 27:21-21|A Statute For Ever": "This lamp service was to remain an ongoing rule for Israel's generations.",
  "Exodus 28:1-6|Holy Garments": "Holy garments are priestly clothes set apart for service before the LORD.",
  "Exodus 28:1-6|For Glory And For Beauty": "The garments are made to display dignity, honor, and fitting beauty in priestly service.",
  "Exodus 28:7-12|Two Shoulderpieces": "Two shoulderpieces join the ephod together at the shoulders.",
  "Exodus 28:7-12|Stones Of Memorial": "The engraved stones serve as a memorial by carrying Israel's names before God.",
  "Exodus 28:27-30|Upon His Heart": "The breastplate rests over Aaron's heart to picture Israel being carried before the LORD.",
};
...
function explainDay27Phrase(section: PersonalExodusPhraseSectionInput, title: string): string {
  ...
  if (lower.includes("judgments")) {
    ...
    add(`${title} treats violence, family dishonor, and kidnapping as serious sins.`, "Israel's law protects life, parents, and human dignity.", "Some actions are not small disputes; they attack the order God gives for community life.", "ðŸ©¸ Life matters", "ðŸ‘¨â€ðŸ‘©â€ðŸ‘§ Family honor", "ðŸš« Kidnapping condemned", "A rescued people must value human life because every person lives before God.");
  } else if (lower.includes("appoint thee a place") || lower.includes("god deliver him")) {
    add(`${title} means God's justice pays attention to intent, guilt, and refuge.`, "Not every death is treated the same way.", "The law makes room for justice that considers intent, guilt, and refuge.", "âš–ï¸ Intent matters", "ðŸƒ Place of refuge", "ðŸ“œ Careful judgment", "God's justice is not careless revenge.");
    ...
    add(`${title} means harm could not be brushed aside as if nothing happened.`, "The law does not pretend harm is harmless.", "Lost time, healing, pregnancy, servants, and life itself all matter before God.", "ðŸ©¹ Healing", "â³ Lost time", "âš–ï¸ Accountability", "Justice means damage must be faced, not ignored.");
    ...
    add(`${title} teaches measured justice, not unlimited revenge.`, "The punishment must fit the harm.", "This law restrains personal vengeance by placing injury under public justice.", "ðŸ‘ï¸ Eye", "ðŸ¦· Tooth", "âš–ï¸ Proportion", "God's law limits retaliation so anger does not become cruelty.");
  } else if (lower.includes("go free")) {
    add(`${title} gives freedom as a form of justice when a servant is injured.`, "The injured person is not simply sent back into the same situation as though nothing happened.", "The law says bodily harm matters.", "ðŸšª Freedom", "ðŸ©¹ Injury recognized", "âš–ï¸ Protection", "God's justice sees the vulnerable person, not only the powerful household.");
    ...
    add(`${title} teaches mercy toward people who can easily be mistreated.`, "Israel knows what it was like to be powerless in Egypt.", "So God commands them not to crush foreigners, widows, or fatherless children.", "ðŸ’” Vulnerable people", "ðŸ‘‚ God hears", "âš–ï¸ Mercy and justice", "The rescued must not become oppressors.");
    ...
    add(`${title} teaches reverence, ordered giving, and holy identity.`, "Israel's speech, offerings, firstborn, and food practices all belong under God's rule.", "Holiness is not only what happens at a mountain. It reaches daily choices.", "ðŸ—£ï¸ Reverent speech", "ðŸŽ First offerings", "âœ¨ Holy people", "God is shaping a community that looks different from Egypt and the nations.");
    ...
    add(`${title} protects justice from lies, pressure, bribery, and partiality.`, "God does not want courts controlled by crowds, money, fear, or favoritism.", "Truth matters whether the person is rich, poor, native, or foreign.", "ðŸ—£ï¸ No false report", "ðŸ’° No bribe", "âš–ï¸ Fair judgment", "Justice must not bend toward the powerful or pretend to help by lying.");
  } else if (lower.includes("enemy's ox") || lower.includes("ass going astray")) {
    add(`${title} teaches kindness even when the owner is an enemy.`, "Israel cannot use personal dislike as an excuse to ignore another person's loss.", "The law trains people to do what is right even when feelings are hard.", "ðŸ‚ Enemy's animal", "ðŸ¤ Return what is lost", "ðŸ’› Mercy", "God's justice reaches beyond people we already like.");
    ...
    add(`${title} connects guidance with loyal worship.`, "God promises to lead Israel, but they must not turn aside to the gods of the land.", "The way forward requires obedience, not just movement.", "ðŸ§­ Guidance", "ðŸ™Œ Worship the LORD", "ðŸš« No idols", "God leads His people toward promise while warning them against divided worship.");
    ...
    add(`${title} means God will bring Israel into the land with care and timing.`, "The LORD promises blessing and victory, but He also says the land will not be cleared all at once.", "Little by little is still God's faithfulness.", "â³ Gradual promise", "ðŸ›¡ï¸ God goes before", "ðŸ“ Set bounds", "God's timing protects His people from dangers they may not even see.");
    ...
    add(`${title} warns Israel against agreements that would pull them into idolatry.`, "The issue is not ordinary kindness to neighbors.", "The danger is covenant loyalty with people and gods that would turn Israel away from the LORD.", "ðŸš« No idolatrous covenant", "âš ï¸ Snare", "ðŸ™Œ Loyalty to God", "A trap can look peaceful if it slowly teaches the heart to worship wrongly.");
    ...
    add(`${title} prepares the covenant ceremony with worship, words, altar, and witness.`, "Moses writes the LORD's words and the people answer together.", "The altar and twelve pillars show covenant worship involving all Israel.", "ðŸ“œ Written words", "ðŸª¨ Altar and pillars", "ðŸ‘¥ One voice", "The people are being formally bound to the God who rescued them.");
  } else if (lower.includes("book of the covenant")) {
    add(`${title} means Moses has written covenant words to read aloud to the people.`, "Israel is not agreeing to vague feelings about God.", "They hear the covenant read aloud so their response is tied to actual words from the LORD.", "ðŸ“– Written covenant", "ðŸ‘‚ Public reading", "ðŸ“œ Clear words", "God's people need His words, not guesswork.");
  } else if (lower.includes("will we do")) {
    add(`${title} is Israel's spoken promise after hearing the covenant read.`, "The people answer that they will obey what the LORD has said.", "Their words are serious because covenant obedience is not casual enthusiasm.", "ðŸ—£ï¸ Public promise", "âœ… We will do", "ðŸ“œ God's commands", "The coming story will show they need grace, but the response here is still a real covenant commitment.");
  } else if (lower.includes("moses took the blood")) {
    add(`${title} begins the blood-sign part of the covenant ceremony.`, "Moses does not treat the covenant as only a speech or agreement.", "Blood from sacrifice marks the seriousness of life with the holy LORD.", "ðŸ©¸ Blood taken", "ðŸ”¥ Sacrifice", "ðŸ“œ Covenant ceremony", "The covenant is sealed with life-and-death seriousness.");
  } else if (lower.includes("sprinkled it on the people")) {
    add(`${title} means the covenant blood is applied to the people, not only the altar.`, "The blood is not kept far away from the people.", "It is sprinkled on them to mark that they are included in the covenant relationship.", "ðŸ©¸ Blood applied", "ðŸ‘¥ People marked", "âœ… Covenant accepted", "Israel is not just hearing about covenant. They are being brought under it.");
  } else if (lower.includes("blood of the covenant")) {
    add(`${title} gives the ceremony its meaning.`, "The blood points to the seriousness of the relationship between the LORD and His rescued people.", "Covenant with God is not a casual handshake. It is sealed through sacrifice.", "ðŸ©¸ Covenant blood", "ðŸ“– Covenant words", "ðŸ™Œ Holy relationship", "These words help later readers understand why covenant, sacrifice, and forgiveness belong together.");
  } else if (lower.includes("concerning all these words")) {
    add(`${title} ties the blood ceremony back to the covenant words.`, "The sacrifice does not float apart from obedience.", "The people are being bound to the LORD according to what He has spoken.", "ðŸ“œ All these words", "ðŸ©¸ Blood seal", "âœ… Covenant responsibility", "God's covenant includes both mercy and a real call to obey.");
    ...
    add(`${title} means Moses is entering the holy cloud where the LORD is present.`, "The mountain is covered with glory, and the people see the LORD's presence like devouring fire.", "Moses stays forty days and nights to receive more instruction.", "â˜ï¸ Cloud", "ðŸ”¥ Devouring fire", "â›°ï¸ Forty days", "God's nearness is beautiful, weighty, and never casual.");
  } else {
    add(`${title} explains one part of Israel's covenant life after Sinai.`, "The wording helps the reader see what God is teaching about justice, mercy, worship, responsibility, or covenant nearness.", "ðŸ“– Scripture wording", "ðŸ” Phrase meaning", "ðŸ§  Covenant understanding", "God's rescued people are learning how to live differently from Egypt.");
  }

  return finalizeExodusPhraseExplanation(title, lines, DAY_27_FIRST_LINE_OVERRIDES, section.reference);
}
...
function explainDay28Phrase(section: PersonalExodusPhraseSectionInput, title: string): string {
  ...
  if (lower.includes("blue, and purple, and scarlet") && section.reference === "Exodus 26:1-6") {
    add(`${title} means these colors are woven into the tabernacle curtains.`, "These colors are not just a supply list here; they are woven into the fabric surrounding holy space.", "The curtains teach beauty, skill, and reverence in the place where God dwells.", "ðŸ§µ Woven colors", "ðŸ•ï¸ Tabernacle curtains", "âœ¨ Holy beauty", "The same colors given as offerings are now shaped into worship space.");
  } else if (lower.includes("shittim wood") && section.reference === "Exodus 27:1-6") {
    add(`${title} means this wood is used for the altar of sacrifice.`, "The altar is made to be strong, portable, and overlaid with brass.", "This is the place where sacrifice is offered before approach to God.", "ðŸªµ Shittim wood", "ðŸ”¥ Altar", "ðŸ©¸ Sacrifice", "The material serves the worship where sin is dealt with before the LORD.");
  } else if (lower.includes("overlay it with pure gold") && section.reference === "Exodus 25:23-28") {
    add(`${title} means the table is covered with gold because it is set apart for holy service.`, "This table is not ordinary household furniture.", "It stands in the tabernacle as part of worship before the LORD.", "âœ¨ Gold covering", "ðŸž Holy table", "ðŸ™Œ Set apart", "The gold marks the table as belonging to sacred service.");
  } else if (lower.includes("crown of gold round about") && section.reference === "Exodus 25:23-28") {
    add(`${title} means the holy table has a finished gold rim around it.`, "The crown gives the table a finished, honored edge.", "Even a border detail teaches that worship objects are carefully made for God's presence.", "ðŸ‘‘ Gold rim", "ðŸž Table", "âœ¨ Careful beauty", "Small details matter when they serve holy worship.");
  } else if (lower.includes("four rings of gold") && section.reference === "Exodus 25:23-28") {
    add(`${title} gives the table a way to be carried without treating it casually.`, "The rings held the poles used for transport.", "Holy furniture could move with Israel, but it had to be carried according to God's design.", "ðŸ’ Gold rings", "ðŸš¶ Carried table", "ðŸ“ Ordered movement", "God's dwelling travels with His people, but His holiness still shapes how they move it.");
  } else if (lower.includes("forty sockets of silver") && section.reference === "Exodus 26:21-26") {
    add(`${title} repeats the silver foundation pieces for another side of the tabernacle.`, "The matching sockets show that the tent's structure is balanced and ordered.", "The repeated detail helps the reader picture a stable holy dwelling, not a loose shelter.", "ðŸ¥ˆ Silver sockets", "ðŸªµ Boards supported", "ðŸ•ï¸ Ordered structure", "God's house is built with care on every side.");
  } else if (lower.includes("statute for ever") && section.reference === "Exodus 28:43-43") {
    add(`${title} makes priestly holiness an ongoing rule, not a one-time warning.`, "The priests must keep these clothing commands whenever they serve.", "Holy service remains serious from generation to generation.", "ðŸ“œ Lasting statute", "ðŸ‘• Priestly covering", "âš ï¸ Holy responsibility", "God's people cannot treat priestly service as casual just because time passes.");
  } else if (lower.includes("glory and for beauty") && section.reference === "Exodus 28:37-42") {
    add(`${title} repeats the purpose of the priestly garments after more clothing details are given.`, "The coats, girdles, bonnets, and covering are not merely practical.", "They show dignity, holiness, and beauty in service before the LORD.", "ðŸ‘• Garments", "âœ¨ Beauty", "ðŸ™Œ Holy service", "Priestly clothing teaches that worship before God should be reverent and fitting.");
  } else if (lower.includes("pure oil olive") || lower.includes("beaten for the light")) {
    add(`${title} means this oil is prepared for the tabernacle lamp.`, "The oil was beaten and prepared so the lamp could keep burning before the LORD.", "This is not only a material; it supports continual light in the holy place.", "ðŸ«’ Olive oil", "ðŸ•¯ï¸ Lamp light", "ðŸŒ™ Steady service", "God's dwelling place is marked by ordered light before His presence.");
  } else if (lower.includes("aaron shall bear their names")) {
    add(`${title} means Aaron carries Israel's tribes before the LORD.`, "The names on the stones mean the priest does not serve as a private person.", "He represents the people of Israel when he comes near to God.", "ðŸ’Ž Names on stones", "ðŸ‘¥ Israel represented", "ðŸ™Œ Priest carries the people", "God's people are remembered before Him in priestly service.");
  } else if (lower.includes("that it may be one")) {
    add(`${title} means the tabernacle pieces are joined into one whole dwelling.`, "The curtains and clasps are not scattered parts.", "They are fastened together so the tent functions as one ordered sanctuary.", "ðŸ•ï¸ One tent", "ðŸ”— Joined pieces", "ðŸ“ Ordered worship", "God's dwelling place is carefully joined, not thrown together.");
  } else if (lower.includes("cubit on the one side") || lower.includes("cubit on the other side") || lower.includes("fifteen cubits")) {
    add(`${title} gives a measurement that helps the reader picture the holy space.`, "Measurements may feel slow, but they show that the tabernacle is built with exact care.", "God's pattern gives shape to every side, length, and boundary.", "ðŸ“ Measurement", "ðŸ•ï¸ Holy space", "ðŸ“ Exact pattern", "The details teach that worship follows God's ordered design.");
  } else if (lower.includes("so shall they make it")) {
    add(`${title} closes the altar instruction by calling for obedience to the shown pattern.`, "The altar is not made by personal preference.", "It must be built the way God showed Moses on the mountain.", "ðŸ“ Shown pattern", "ðŸ”¥ Altar", "âœ… Obedient building", "Holy worship is shaped by God's command from start to finish.");
  } else if (lower === "before the lord") {
    add(`${title} places the lamp service in God's presence.`, "The lamp is not burning as decoration for people first.", "It burns before the LORD as part of faithful tabernacle service.", "ðŸ•¯ï¸ Lamp", "ðŸ™Œ Before God", "ðŸŒ™ Evening to morning", "The phrase reminds the reader that worship is done in the sight of the LORD.");
  } else if (lower.includes("that he may minister unto me")) {
    add(`${title} explains why Aaron is set apart.`, "Priesthood is not mainly about honor in front of people.", "Aaron is chosen to minister to the LORD in holy service.", "ðŸ™Œ Ministry to God", "ðŸ‘• Priestly calling", "âœ¨ Set apart", "The priest serves before God for the sake of the people.");
  } else if (lower === "at the ends") {
    add(`${title} explains where the gold chains attach on the priestly garment.`, "The phrase is small, but it helps picture how the breastpiece and ephod are fastened.", "Priestly clothing is carefully joined so it can be worn in holy service.", "ðŸ”— Chain ends", "ðŸ‘• Garment pieces", "ðŸ“ Careful design", "Even connecting points matter in work done before the LORD.");
  } else if (lower.includes("every one with his name") || lower.includes("engravings of a signet")) {
    add(`${title} means each tribe is personally named before God.`, "The stones are engraved like official seals, not loosely marked.", "Every name matters because the priest carries real people before the LORD.", "âœï¸ Engraved names", "ðŸ’Ž Tribal stones", "ðŸ‘¥ Every tribe remembered", "God's people are represented specifically, not vaguely.");
  } else if (lower.includes("bear not iniquity")) {
    add(`${title} warns that priestly service carries real guilt if handled carelessly.`, "The clothing commands are tied to holiness, not appearance only.", "The priests must serve covered and prepared so they do not bear iniquity before the LORD.", "âš ï¸ Iniquity", "ðŸ‘• Priestly covering", "ðŸ™‡ Holy service", "Serving near God's presence is a gift, but it is never casual.");
  } else if (lower.includes("offering") || lower.includes("willingly") || lower.includes("heart")) {
    add(`${title} means the tabernacle begins with willing gifts, not forced labor.`, "Egypt took work from Israel under pressure, but the LORD invites His rescued people to give from the heart.", "The materials for worship come through willing participation.", "ðŸŽ Offering", "ðŸ’› Willing heart", "ðŸ™Œ Worship response", "God is forming a worshiping people, not another slave workforce.");
    ...
    add(`${title} means the ark area joins God's word, mercy, and presence.`, "The testimony holds God's covenant words, and the mercy seat becomes the place where God promises to meet and speak.", "The cherubim show that this is holy space, not casual furniture.", "ðŸ“œ Testimony", "ðŸª½ Cherubim", "ðŸ™Œ God meets and speaks", "At the center of worship is the LORD's mercy and His word.");
    ...
    add(`${title} means the table is holy furniture for service before the LORD.`, "The table is carefully built, bordered, overlaid, and carried like other holy furniture.", "Even practical details teach order and reverence.", "ðŸž Holy table", "âœ¨ Gold", "ðŸš¶ Carried carefully", "God's dwelling place is beautiful, ordered, and treated with care.");
  } else if (lower.includes("dishes") || lower.includes("spoons") || lower.includes("covers") || lower.includes("bowls") || lower.includes("shewbread")) {
    add(`${title} names items connected to the bread set before the LORD.`, "The shewbread reminded Israel that life and provision are lived before God.", "The dishes and bowls show that worship had ordinary-looking objects set apart for holy use.", "ðŸž Bread before God", "ðŸ¥£ Holy vessels", "ðŸ™Œ Provision remembered", "The LORD's house teaches fellowship and dependence.");
    ...
    add(`${title} means this layer covers and protects the tabernacle.`, "The holy tent needed coverings that protected and completed the structure.", "Even outer layers mattered because the dwelling place of God was treated with care.", "ðŸ•ï¸ Covering", "ðŸ Goats' hair", "ðŸ›¡ï¸ Protection", "Holy space is both beautiful inside and carefully covered outside.");
    ...
    add(`${title} gives the tabernacle structure strength and shape.`, "Boards, sockets, tenons, rings, and bars hold the tent together.", "The details may feel technical, but they show that God's dwelling place is ordered and stable.", "ðŸªµ Boards", "âš™ï¸ Sockets and rings", "ðŸ•ï¸ Strong structure", "Worship is not thrown together. It is built according to God's pattern.");
    ...
    add(`${title} teaches boundaries inside holy space.`, "The veil divided the Holy Place from the Most Holy Place.", "God is near His people, but His holiness still creates ordered access.", "ðŸšª Veil", "ðŸ“ Holy and Most Holy", "ðŸ™‡ Reverent approach", "The tabernacle says both truths at once: God dwells near, and God is holy.");
    ...
    add(`${title} means this detail helps form the courtyard boundary around the tabernacle.`, "The court marked off holy space from ordinary camp space.", "People did not rush into God's presence however they wanted.", "ðŸš§ Boundary", "ðŸ•ï¸ Court", "ðŸšª Gate", "God's nearness is open by His command and guarded by His holiness.");
    ...
    add(`${title} teaches steady light before the LORD.`, "The lamp was to burn from evening to morning outside the veil.", "Aaron and his sons were responsible for this regular service.", "ðŸ•¯ï¸ Lamp", "ðŸŒ™ Evening to morning", "ðŸ™Œ Continual service", "God's people learn that worship includes faithful rhythms, not only dramatic moments.");
    ...
    add(`${title} introduces Aaron and his sons as priests set apart for holy service.`, "The garments are not costumes for showing off.", "They mark the priest as someone who represents Israel before the LORD.", "ðŸ‘• Holy garments", "ðŸ™Œ Priest service", "âœ¨ Glory and beauty", "Coming near to God requires a God-appointed mediator.");
    ...
    add(`${title} means the breastplate is worn over the priest's heart as he represents Israel.`, "The precious stones represent Israel before the LORD in beauty and order.", "The breastplate is tied to judgment because the priest serves as representative before God.", "ðŸ’Ž Precious stones", "â¤ï¸ Over the heart", "âš–ï¸ Judgment", "God's people are carried before Him, not forgotten.");
    ...
    add(`${title} means Aaron carries Israel's judgment before the LORD continually.`, "The priest bears the names and the judgment of the people near God's presence.", "This is a picture of representation, responsibility, and holy mediation.", "â¤ï¸ Upon his heart", "ðŸ‘¥ Israel represented", "ðŸ™Œ Before the LORD", "The priest's work teaches that God's people need someone to stand before God for them.");
    ...
    add(`${title} means priestly service is visible, audible, and holy.`, "The robe, bells, pomegranates, and golden plate all mark Aaron's service before the LORD.", "HOLINESS TO THE LORD names the whole point of the priestly calling.", "ðŸ”” Bells", "ðŸŽ Pomegranates", "âœ¨ Holiness", "The priest approaches God as one set apart for holy service.");
    ...
    add(`${title} means serving near God's holiness is serious.`, "The priestly clothing covers, marks, and prepares the priests for service.", "Careless approach could bring guilt and death because the work is before the holy LORD.", "ðŸ‘• Priestly clothing", "âš ï¸ Holy danger", "âœ… Accepted before God", "The details are not fashion notes. They teach reverence before God's presence.");
  } else {
    add(`${title} explains one piece of the pattern God gave Moses.`, "The tabernacle is not built from Israel's imagination.", "Its objects, materials, priests, and boundaries are all received from the LORD.", "ðŸ“– Scripture wording", "ðŸ” Phrase meaning", "ðŸ™Œ Holy worship", "The rescued people are learning how a holy God dwells among them.");
  }

  return finalizeExodusPhraseExplanation(title, lines, DAY_28_FIRST_LINE_OVERRIDES, section.reference);
}
...
function explainExodusTwentyNineToThirtyPhrase(section: PersonalExodusPhraseSectionInput, title: string): string {
  ...
  return finalizeExodusPhraseExplanation(title, lines, undefined, section.reference);
}
...
function deepenDay29Exodus29To30Mining(section: PersonalExodusPhraseSectionInput): PersonalExodusPhraseSectionInput {
  ...
  const existing = new Set(section.phrases.map(([phraseTitle]) => phraseTitle.replace(/^ðŸ“Œ\s*/, "").trim().toLowerCase()));
...
const EXODUS_21_30_MOBILE_FORMAT_CUES: Record<number, string[]> = {
  21: ["âš–ï¸ These laws limit harm and protect life.", "ðŸ  Covenant justice reaches households and daily conflicts.", "ðŸ›¡ï¸ God cares about people with less power."],
  22: ["ðŸ§¾ Restitution repairs what theft or damage broke.", "ðŸ’” Widows, orphans, strangers, and the poor receive special protection.", "ðŸ™Œ Holiness includes how people treat neighbors."],
  23: ["ðŸ—£ï¸ Truth matters in court and community life.", "ðŸ›‘ Sabbath and feasts shape Israel's time around God.", "ðŸ§­ The promised land must not pull Israel into false worship."],
  24: ["ðŸ“œ Israel hears the covenant words.", "ðŸ©¸ Blood seals the covenant relationship.", "â›°ï¸ Moses goes up because God's presence is holy."],
  25: ["ðŸŽ Worship begins with willing gifts.", "ðŸ“¦ The ark and mercy seat sit at the center.", "ðŸ•ï¸ God wants to dwell among His people."],
  26: ["ðŸ•ï¸ The tent teaches holy space.", "ðŸšª The veil shows nearness and separation.", "ðŸ§µ Beauty and order belong in worship."],
  27: ["ðŸ©¸ The altar teaches sacrifice before approach.", "ðŸš§ The courtyard marks a holy boundary.", "ðŸ•¯ï¸ The lamp keeps light before the LORD."],
  28: ["ðŸ‘• Priestly garments teach holy service.", "ðŸ’Ž Israel's names are carried before God.", "ðŸ™Œ The priest represents the people."],
  29: ["ðŸ’§ Priests must be washed and set apart.", "ðŸ©¸ Sacrifice deals with sin before service.", "ðŸ•ï¸ God meets His people at the tent."],
  30: ["ðŸ•¯ï¸ Incense marks daily worship near the veil.", "ðŸ’§ Washing teaches cleansing before service.", "ðŸ§´ Holy oil and incense are set apart for God."],
};
...
function getExodusTwentyOneToThirtyPhraseList(section: PersonalExodusPhraseSectionInput, cleanTitle: string) {
  ...
  if (/servant|maid|master|manservant|bondman|bondwoman|slave/.test(lower)) {
    return [
      `âš–ï¸ ${cleanTitle}`,
      "ðŸ  Justice inside daily life",
      "ðŸ›¡ï¸ Protection for the vulnerable",
      "ðŸ“œ Covenant law shaping conduct",
    ];
  }
  ...
  if (/ox|sheep|ass|beast|field|vineyard|fire|pit|restitution|restore/.test(lower)) {
    return [
      `ðŸ§¾ ${cleanTitle}`,
      "ðŸ‚ Property and responsibility",
      "âš–ï¸ Repairing harm",
      "ðŸ  Neighbor love in practical form",
    ];
  }
  ...
  if (/widow|fatherless|stranger|poor|enemy|neighbor|borrow|pledge/.test(lower)) {
    return [
      `ðŸ›¡ï¸ ${cleanTitle}`,
      "ðŸ’” People with less power",
      "ðŸ¤ Mercy in the community",
      "ðŸ™Œ God's concern for justice",
    ];
  }
  ...
  if (/sabbath|feast|firstfruits|bread|altar|sacrifice|blood|covenant/.test(lower)) {
    return [
      `ðŸ™Œ ${cleanTitle}`,
      "ðŸ“… Worship shaping time",
      "ðŸ©¸ Covenant relationship",
      "ðŸ“œ Israel living before the LORD",
    ];
  }
  ...
  if (/ark|mercy seat|cherubim|table|candlestick|lamp|tabernacle|curtain|veil|altar|court|ephod|breastplate|robe|garment/.test(lower)) {
    return [
      `ðŸ•ï¸ ${cleanTitle}`,
      "ðŸ§µ Holy design",
      "ðŸ™Œ Worship near God's presence",
      "ðŸ“œ Every detail given by command",
    ];
  }
  ...
  if (/priest|aaron|sons|holy|anoint|consecrate|wash|incense|oil/.test(lower)) {
    return [
      `ðŸ•¯ï¸ ${cleanTitle}`,
      "ðŸ§¼ Set apart for service",
      "ðŸ™Œ Priestly worship",
      "ðŸ“œ Holiness before the LORD",
    ];
  }

  return [
    `ðŸ”Ž ${cleanTitle}`,
    section.chapter <= 24 ? "âš–ï¸ Covenant life being shaped" : "ðŸ•ï¸ Worship space being prepared",
    section.chapter <= 24 ? "ðŸ  God's law reaching daily life" : "ðŸ™Œ God teaching Israel how to approach",
  ];
}
...
function getDays27To29TeachingBullets(section: PersonalExodusPhraseSectionInput, title: string) {
  ...
  if (section.chapter >= 21 && section.chapter <= 24) {
    if (/servant|bond|maid|master|ox|thief|borrow|pledge|widow|fatherless|stranger|judge|rest|sabbath|feast|blood|altar|book/.test(lower)) {
      return ["âš–ï¸ God's law protects real people", "ðŸ  Covenant life reaches ordinary situations", "ðŸ›¡ï¸ The weak must not be exploited", "ðŸ“œ Israel must live under the LORD's justice"];
    }
    return ["ðŸ“œ The LORD gives covenant instruction", "âš–ï¸ Justice matters among God's people", "ðŸ¤ Neighbor love becomes practical", "ðŸ™Œ Rescue leads into ordered obedience"];
  }
  ...
  if (section.chapter >= 25 && section.chapter <= 28) {
    if (/ark|mercy seat|cherubim|table|candlestick|lamp|curtain|vail|altar|court|oil|ephod|breastplate|stones|garments|mitre|plate/.test(lower)) {
      return ["ðŸ•ï¸ The tabernacle teaches holy nearness", "âœ¨ Worship has beauty and order", "ðŸ©¸ Access to God requires His provision", "ðŸ‘‘ The priest represents the people before the LORD"];
    }
    return ["ðŸ•ï¸ God is preparing a dwelling among Israel", "ðŸ› ï¸ The details follow His command", "ðŸ™Œ Worship is shaped by God's holiness", "âœ¨ Beauty serves the presence of the LORD"];
  }
  ...
  if (section.chapter >= 29 && section.chapter <= 30) {
    if (/priest|aaron|sons|blood|ram|altar|anoint|atonement|incense|wash|basin|holy|continual|offering/.test(lower)) {
      return ["ðŸ©¸ Blood marks consecration and atonement", "ðŸ‘‘ Priests are set apart for holy service", "ðŸ”¥ Worship must be offered God's way", "ðŸ™Œ The LORD makes a way to dwell with His people"];
    }
    return ["â›º Holy service happens by God's command", "ðŸ§¼ Cleansing is required near His presence", "ðŸ•¯ï¸ Worship continues before the LORD", "ðŸ“œ Israel cannot invent its own approach to God"];
  }

  return ["ðŸ“– The phrase comes from the assigned text", "ðŸ” It explains a real detail in the verse", "ðŸ™Œ The LORD is teaching His people", "ðŸ§  The meaning should be clear before moving on"];
}
...
function makeDays27To29OpeningLineUnique(line: string, title: string, section: PersonalExodusPhraseSectionInput) {
  const keyedOverrides: Record<string, string> = {
    "Exodus 32:27-29|About Three Thousand Men": "The judgment after the calf struck about three thousand men.",
  };
  const keyed = keyedOverrides[`${section.reference}|${title}`];
  if (keyed) return keyed;

  return stripTitleFromOpening(title, line);
}
...
export const EXODUS_21_30_PERSONAL_SECTIONS: PersonalExodusPhraseSectionInput[] = formatExodusTwentyOneToThirtySectionExplanations([
  ...makeExodusSectionsFromDeepStudy(BIBLE_YEAR_DAY_TWENTY_SEVEN_DEEP_STUDY_SECTIONS, [21, 22, 23, 24], "âš–ï¸").map(deepenDay27PhraseCards),
  ...makeExodusSectionsFromDeepStudy(BIBLE_YEAR_DAY_TWENTY_EIGHT_DEEP_STUDY_SECTIONS, [25, 26, 27, 28], "â›º").map(deepenDay28PhraseCards),
  ...makeExodusSectionsFromDeepStudy(BIBLE_YEAR_DAY_TWENTY_NINE_DEEP_STUDY_SECTIONS, [29, 30], "ðŸ•¯ï¸").map(deepenDay29Exodus29To30PhraseCards).map(deepenDay29Exodus29To30Mining),
