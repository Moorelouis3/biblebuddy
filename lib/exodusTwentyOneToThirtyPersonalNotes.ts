...
function explainDay27Phrase(section: PersonalExodusPhraseSectionInput, title: string): string {
  ...
  if (lower.includes("judgments")) {
    ...
    add(`${title} teaches that servitude in Israel had limits and protections.`, "This is not Pharaoh-style slavery where people are crushed without release.", "The law puts boundaries around service and gives a path to freedom.", "⏳ Six years", "🚪 Release", "🛡️ Protected servant", "God's rescued people must not treat workers the way Egypt treated them.");
    ...
    add(`${title} shows that God's law distinguishes between murder and other kinds of death.`, "Not every death is treated the same way.", "The law makes room for justice that considers intent, guilt, and refuge.", "⚖️ Intent matters", "🏃 Place of refuge", "📜 Careful judgment", "God's justice is not careless revenge.");
    ...
    add(`${title} shows that injury creates responsibility.`, "The law does not pretend harm is harmless.", "Lost time, healing, pregnancy, servants, and life itself all matter before God.", "🩹 Healing", "⏳ Lost time", "⚖️ Accountability", "Justice means damage must be faced, not ignored.");
    ...
    add(`${title} teaches that theft and property damage require restoration.`, "The law does not only punish the thief; it aims to repair what was taken or ruined.", "Oxen, sheep, fields, and fire damage all affected survival and livelihood.", "🐑 Stolen animals", "🌾 Damaged field", "🔁 Restitution", "God's justice pushes wrongdoers toward repair, not excuses.");
    ...
    add(`${title} teaches that God's promise will unfold with care and timing.`, "The LORD promises blessing and victory, but He also says the land will not be cleared all at once.", "Little by little is still God's faithfulness.", "⏳ Gradual promise", "🛡️ God goes before", "📍 Set bounds", "God's timing protects His people from dangers they may not even see.");
    ...
    add(`${title} names the written covenant words Moses reads to the people.`, "Israel is not agreeing to vague feelings about God.", "They hear the covenant read aloud so their response is tied to actual words from the LORD.", "📖 Written covenant", "👂 Public reading", "📜 Clear words", "God's people need His words, not guesswork.");
    ...
    add(`${title} shows the covenant sign being applied to Israel.`, "The blood is not kept far away from the people.", "It is sprinkled on them to mark that they are included in the covenant relationship.", "🩸 Blood applied", "👥 People marked", "✅ Covenant accepted", "Israel is not just hearing about covenant. They are being brought under it.");
    ...
    add(`${title} shows Moses entering the holy cloud of God's presence.`, "The mountain is covered with glory, and the people see the LORD's presence like devouring fire.", "Moses stays forty days and nights to receive more instruction.", "☁️ Cloud", "🔥 Devouring fire", "⛰️ Forty days", "God's nearness is beautiful, weighty, and never casual.");
  } else {
...
function explainDay28Phrase(section: PersonalExodusPhraseSectionInput, title: string): string {
  ...
  if (lower.includes("blue, and purple, and scarlet") && section.reference === "Exodus 26:1-6") {
    add(`${title} describes the colors woven into the tabernacle curtains.`, "These colors are not just a supply list here; they become part of the fabric surrounding holy space.", "The curtains teach beauty, skill, and reverence in the place where God dwells.", "🧵 Woven colors", "🏕️ Tabernacle curtains", "✨ Holy beauty", "The same colors given as offerings are now shaped into worship space.");
  } else if (lower.includes("shittim wood") && section.reference === "Exodus 27:1-6") {
    add(`${title} names the wood used for the altar of sacrifice.`, "The altar is made to be strong, portable, and overlaid with brass.", "This is the place where sacrifice is offered before approach to God.", "🪵 Shittim wood", "🔥 Altar", "🩸 Sacrifice", "The material serves the worship where sin is dealt with before the LORD.");
  } else if (lower.includes("overlay it with pure gold") && section.reference === "Exodus 25:23-28") {
    add(`${title} shows the table being covered with gold for holy service.`, "This table is not ordinary household furniture.", "It stands in the tabernacle as part of worship before the LORD.", "✨ Gold covering", "🍞 Holy table", "🙌 Set apart", "The gold marks the table as belonging to sacred service.");
  } else if (lower.includes("crown of gold round about") && section.reference === "Exodus 25:23-28") {
    add(`${title} describes the gold rim around the holy table.`, "The crown gives the table a finished, honored edge.", "Even a border detail teaches that worship objects are carefully made for God's presence.", "👑 Gold rim", "🍞 Table", "✨ Careful beauty", "Small details matter when they serve holy worship.");
    ...
    add(`${title} shows that each tribe is personally named before God.`, "The stones are engraved like official seals, not loosely marked.", "Every name matters because the priest carries real people before the LORD.", "✍️ Engraved names", "💎 Tribal stones", "👥 Every tribe remembered", "God's people are represented specifically, not vaguely.");
    ...
    add(`${title} points to the meeting place above the ark.`, "The testimony holds God's covenant words, and the mercy seat becomes the place where God promises to meet and speak.", "The cherubim show that this is holy space, not casual furniture.", "📜 Testimony", "🪽 Cherubim", "🙌 God meets and speaks", "At the center of worship is the LORD's mercy and His word.");
    ...
    add(`${title} describes the altar where sacrifice is offered before approach.`, "The altar stands in the worship system because sinful people need atonement before a holy God.", "Its horns, vessels, ashes, and carrying poles all serve that holy work.", "🩸 Sacrifice", "🔥 Altar", "📐 Mountain pattern", "Approach to God begins with God's provided way, not human confidence.");
    ...
    add(`${title} describes the courtyard boundary around the tabernacle.`, "The court marked off holy space from ordinary camp space.", "People did not rush into God's presence however they wanted.", "🚧 Boundary", "🏕️ Court", "🚪 Gate", "God's nearness is open by His command and guarded by His holiness.");
    ...
    add(`${title} describes the breastplate worn over the priest's heart.`, "The precious stones represent Israel before the LORD in beauty and order.", "The breastplate is tied to judgment because the priest serves as representative before God.", "💎 Precious stones", "❤️ Over the heart", "⚖️ Judgment", "God's people are carried before Him, not forgotten.");
    ...
    add(`${title} shows the seriousness of serving near God's holiness.`, "The priestly clothing covers, marks, and prepares the priests for service.", "Careless approach could bring guilt and death because the work is before the holy LORD.", "👕 Priestly clothing", "⚠️ Holy danger", "✅ Accepted before God", "The details are not fashion notes. They teach reverence before God's presence.");
  } else {
    add(`${title} teaches a real detail about the tabernacle, priesthood, or holy worship.`, "The wording helps a beginner see how God provides a place, pattern, objects, and priests for His presence among Israel.", "📖 Bible words", "🔍 Slow reading", "🙌 Holy worship", "The rescued people are learning how a holy God dwells among them.");
  }
...
function explainExodusTwentyNineToThirtyPhrase(section: PersonalExodusPhraseSectionInput, title: string): string {
  ...
  if (lower === "the ram of consecration" && section.reference === "Exodus 29:16-21") {
    ...
    add("A Sweet Savour describes the burnt offering as pleasing before the LORD.", "This does not mean God needed the smell like a person needs food. It means the offering was accepted according to His command.", "\u{1F525} Offering burned", "\u{1F64C} Accepted worship", "\u{1F4DC} God's way", "The phrase helps beginners see that sacrifice was about relationship with God, not empty ritual.");
  } else if (lower === "a sweet savour") {
    add("A Sweet Savour describes the daily offering rising before God in an accepted way.", "Morning and evening worship taught Israel that life with the LORD had a steady rhythm.", "\u{1F305} Morning", "\u{1F307} Evening", "\u{1F525} Offering", "The repeated offering kept reminding the people that nearness to God depended on His mercy.");
    ...
    add(`${title} means someone or something is being set apart for God.`, "In Exodus, holy does not mean fancy or mysterious. It means belonging to the LORD for His purpose.", "\u{1F64C} Set apart", "\u{1F3D5}\u{FE0F} Used for worship", "\u{26A0}\u{FE0F} Not treated as common", "The priests, altar, oil, garments, and tabernacle are being marked as God's, not Israel's personal property.");
  } else if (lower.includes("wash") || lower.includes("water") || lower.includes("laver")) {
    add(`${title} teaches cleansing before coming near to serve.`, "The priests could not walk into holy work as if uncleanness did not matter.", "\u{1F4A7} Water", "\u{1F9FC} Cleansing", "\u{1F590}\u{FE0F} Hands and feet", "This was a visible lesson: God invites service, but He also teaches reverence.");
    ...
    add(`${title} names sacrifice connected to sin, cleansing, and worship.`, "Even the priests needed sacrifice before they served. Their position did not make them clean by themselves.", "\u{1F402} Animal sacrifice", "\u{1FA78} Blood", "\u{1F64F} Atonement", "A beginner should see the order: God deals with sin before people draw near in worship.");
  } else if (lower.includes("ear") || lower.includes("thumb") || lower.includes("toe")) {
    add(`${title} marks the priest's whole life for God's service.`, "The ear, hand, and foot picture the life of the priest.", "\u{1F442} What he hears", "\u{1F590}\u{FE0F} What he does", "\u{1F463} Where he walks", "Priestly service was not just a job at the altar. God was claiming the servant's listening, working, and walking.");
    ...
    add(`${title} describes part of the offering lifted or assigned in worship.`, "These details can feel strange at first because modern readers do not live around temple sacrifices.", "\u{1F64C} Lifted before God", "\u{1F402} Taken from the sacrifice", "\u{1F372} Shared with the priests when allowed", "The point is that worship included giving God what He commanded, not inventing a simpler version.");
    ...
    add(`${title} warns Israel not to treat holy things like personal products.`, "God's oil and incense were not recipes for everyday use.", "\u{1F6AB} Not copied", "\u{1F6AB} Not used casually", "\u{26A0}\u{FE0F} Serious boundary", "The LORD was teaching His people that worship is received from Him, not customized for human taste.");
    ...
    add(`${title} shows worship built into Israel's daily rhythm.`, "God was not teaching Israel to seek Him only during emergencies.", "\u{1F305} Morning", "\u{1F307} Evening", "\u{1F56F}\u{FE0F} Steady worship", "The people were learning that life with the LORD shapes ordinary time, not only big moments.");
    ...
    add(`${title} is one of the clearest promises in the tabernacle instructions.`, "The goal is not furniture by itself. The goal is God living among His rescued people.", "\u{1F3D5}\u{FE0F} Dwelling place", "\u{1F64C} God comes near", "\u{1F465} Israel is His people", "Exodus began with Israel groaning in slavery. Now the LORD is preparing a place where He will meet them.");
  } else if (lower.includes("unleavened") || lower.includes("loaf") || lower.includes("bread") || lower.includes("eat") || lower.includes("boil")) {
    add(`${title} names food used inside the consecration ceremony.`, "In the Bible, eating can show fellowship, participation, and receiving what God provides.", "\u{1F35E} Bread", "\u{1F372} Shared meal", "\u{1F3D5}\u{FE0F} Holy place", "The priests were not just watching a ritual happen. They were being brought into the service God assigned them.");
    ...
    add(`${title} names a material or design detail for holy furniture.`, "The tabernacle furniture was built with care because it served worship near God's presence.", "\u{1FAB5} Wood", "\u{1F7E1} Gold", "\u{1F3D5}\u{FE0F} Holy furniture", "These details help beginners see that God gave Israel a pattern, not a vague idea.");
  } else {
    add(`${title} names a real part of priestly worship and tabernacle service.`, "Exodus is showing how a holy God can live among a rescued people.", "\u{1F3D5}\u{FE0F} Holy place", "\u{1F64C} Holy service", "\u{1FA78} Sin dealt with", "The details may feel small, but together they teach reverence, cleansing, sacrifice, and God's nearness.");
  }
