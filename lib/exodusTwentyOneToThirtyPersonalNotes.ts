...
function explainExodusTwentyNineToThirtyPhrase(section: PersonalExodusPhraseSectionInput, title: string): string {
  ...
  if (lower === "the ram of consecration" && section.reference === "Exodus 29:16-21") {
    ...
    add(`${title} means cleansing came before holy service.`, "The priests could not walk into holy work as if uncleanness did not matter.", "\u{1F4A7} Water", "\u{1F9FC} Cleansing", "\u{1F590}\u{FE0F} Hands and feet", "This was a visible lesson: God invites service, but He also teaches reverence.");
