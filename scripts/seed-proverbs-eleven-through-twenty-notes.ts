...
import { renderProverbsNotes } from "../lib/proverbsElevenToTwentyNotes";
...
async function main() {
  ...
  for (let chapter = 11; chapter <= 20; chapter += 1) {
  ...
  if (error) {
    console.error("Failed to seed Proverbs 11-20 notes:", error);
    process.exit(1);
  }

  console.log("Seeded deeper Proverbs 11-20 notes into bible_notes.");
}
