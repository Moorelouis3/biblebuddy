...
const MP3_KBPS = 96;
const SCRIPT_PATH = join(process.cwd(), "docs", "how-to-explain-why-you-are-christian-narrator-script.md");
const OUTPUT_PATH = join(process.cwd(), "public", "audio", "bible-topics", "defend-faith", "how-to-explain-why-you-are-christian-narrator-only.mp3");
...
async function generateOpenAiSpeechPcm(text: string) {
  ...
  const response = await fetch("https://api.openai.com/v1/audio/speech", {
    ...
    body: JSON.stringify({
      ...
      input: text,
      instructions:
        "Speak in a deep, warm, calm male Bible Buddy narrator voice. This is a thoughtful Christian apologetics explainer, so keep the tone conversational, confident, gentle, respectful, biblical, and clear. Do not sound angry, rushed, robotic, academic, salesy, or like a loud preacher. Let the resurrection evidence and Scripture references breathe naturally.",
      response_format: "pcm",
    }),
  });
...
async function main() {
  ...
  console.log(`[EXPLAIN_CHRISTIAN] Text chars: ${text.length}, chunks: ${chunks.length}`);
  for (const [index, chunk] of chunks.entries()) {
    console.log(`[EXPLAIN_CHRISTIAN] Generating narrator chunk ${index + 1}/${chunks.length}`);
  ...
  console.log(`[EXPLAIN_CHRISTIAN] Local narrator-only MP3: ${OUTPUT_PATH}`);
  console.log("[EXPLAIN_CHRISTIAN] Done");
}

main().catch((error) => {
  console.error("[EXPLAIN_CHRISTIAN] Failed:", error);
  process.exit(1);
});
