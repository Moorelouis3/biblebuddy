...
type BookName = "Job" | "Psalms" | "Proverbs" | "Ecclesiastes";
...
const proverbChapters = Array.from({ length: 25 }, (_, index) => index + 7);
const ecclesiastesChapters = Array.from({ length: 5 }, (_, index) => index + 1);
...
async function main() {
  const proverbSections = await buildBook("Proverbs", proverbChapters);
  const ecclesiastesSections = await buildBook("Ecclesiastes", ecclesiastesChapters);

  writeFileSync(
    "lib/proverbsSevenToThirtyOnePersonalNotes.ts",
    `import type { PersonalLeviticusPhraseSectionInput } from "./leviticusOneToTenPersonalNotes";

type ProverbsPhraseSectionInput = PersonalLeviticusPhraseSectionInput & { book: "Proverbs" };

export const PROVERBS_7_31_PERSONAL_SECTIONS = [
${serialize(proverbSections)}
] as const satisfies readonly ProverbsPhraseSectionInput[];
`,
  );

  writeFileSync(
    "lib/ecclesiastesOneToFivePersonalNotes.ts",
    `import type { PersonalLeviticusPhraseSectionInput } from "./leviticusOneToTenPersonalNotes";

type EcclesiastesPhraseSectionInput = PersonalLeviticusPhraseSectionInput & { book: "Ecclesiastes" };

export const ECCLESIASTES_1_5_PERSONAL_SECTIONS = [
${serialize(ecclesiastesSections)}
] as const satisfies readonly EcclesiastesPhraseSectionInput[];
`,
  );

  console.log(`Wrote ${proverbSections.length} Proverbs sections.`);
