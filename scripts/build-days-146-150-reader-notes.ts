...
const psalmChapters = Array.from({ length: 15 }, (_, index) => index + 52);
...
function explainPsalm(phrase: string) {
  ...
  if (/boastest thou thyself|goodness of god endureth|green olive tree|trusted in the abundance|deceitful tongue/.test(lower)) {
    return note([
      ...
      "Psalm 52 exposes the person who trusts in harm, lies, and power, then answers with trust in God's mercy.",
...
async function main() {
  ...
  writeFileSync(
    "lib/psalmsFiftyTwoToSixtySixPersonalNotes.ts",
...
export const PSALMS_52_66_PERSONAL_SECTIONS = [
