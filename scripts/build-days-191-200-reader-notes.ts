...
type BookName = "Job" | "Psalms" | "Proverbs" | "Ecclesiastes" | "Song of Solomon" | "Isaiah";
...
const ecclesiastesChapters = Array.from({ length: 7 }, (_, index) => index + 6);
const songChapters = Array.from({ length: 8 }, (_, index) => index + 1);
const isaiahChapters = Array.from({ length: 15 }, (_, index) => index + 1);
...
async function main() {
  const ecclesiastesSections = await buildBook("Ecclesiastes", ecclesiastesChapters);
  const songSections = await buildBook("Song of Solomon", songChapters);
  const isaiahSections = await buildBook("Isaiah", isaiahChapters);

  writeFileSync(
    "lib/ecclesiastesSixToTwelvePersonalNotes.ts",
    `import type { PersonalLeviticusPhraseSectionInput } from "./leviticusOneToTenPersonalNotes";

type EcclesiastesPhraseSectionInput = PersonalLeviticusPhraseSectionInput & { book: "Ecclesiastes" };

export const ECCLESIASTES_6_12_PERSONAL_SECTIONS = [
${serialize(ecclesiastesSections)}
] as const satisfies readonly EcclesiastesPhraseSectionInput[];
`,
  );

  writeFileSync(
    "lib/songOfSolomonOneToEightPersonalNotes.ts",
    `import type { PersonalLeviticusPhraseSectionInput } from "./leviticusOneToTenPersonalNotes";

type SongOfSolomonPhraseSectionInput = PersonalLeviticusPhraseSectionInput & { book: "Song of Solomon" };

export const SONG_OF_SOLOMON_1_8_PERSONAL_SECTIONS = [
${serialize(songSections)}
] as const satisfies readonly SongOfSolomonPhraseSectionInput[];
`,
  );

  writeFileSync(
    "lib/isaiahOneToFifteenPersonalNotes.ts",
    `import type { PersonalLeviticusPhraseSectionInput } from "./leviticusOneToTenPersonalNotes";

type IsaiahPhraseSectionInput = PersonalLeviticusPhraseSectionInput & { book: "Isaiah" };

export const ISAIAH_1_15_PERSONAL_SECTIONS = [
${serialize(isaiahSections)}
] as const satisfies readonly IsaiahPhraseSectionInput[];
`,
  );

  console.log(`Wrote ${ecclesiastesSections.length} Ecclesiastes sections.`);
  console.log(`Wrote ${songSections.length} Song of Solomon sections.`);
