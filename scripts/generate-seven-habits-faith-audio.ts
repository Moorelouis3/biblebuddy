...
const MP3_KBPS = 96;
const SCRIPT_PATH = join(process.cwd(), "docs", "seven-habits-that-weaken-your-faith-script.md");
const OUTPUT_PATH = join(process.cwd(), "public", "audio", "bible-topics", "seven-habits-faith", "seven-habits-that-weaken-your-faith-narrator-only.mp3");
...
async function generateOpenAiSpeechPcm(text: string) {
  ...
  const response = await fetch("https://api.openai.com/v1/audio/speech", {
    ...
    body: JSON.stringify({
      ...
      input: text,
      instructions:
        "Speak in a deep, warm, calm male Bible Buddy narrator voice. This is a list-style faith-strengthening video, so clearly announce each habit header as part of the narration. Keep the tone conversational, grounded, biblical, and encouraging. Do not sound rushed, robotic, academic, salesy, or like a loud preacher. Let Scripture references and practical moments breathe.",
      response_format: "pcm",
    }),
  });
...
async function main() {
  ...
  console.log(`[SEVEN_HABITS_FAITH] Text chars: ${text.length}, chunks: ${chunks.length}`);
  for (const [index, chunk] of chunks.entries()) {
    console.log(`[SEVEN_HABITS_FAITH] Generating narrator chunk ${index + 1}/${chunks.length}`);
  ...
  console.log(`[SEVEN_HABITS_FAITH] Local narrator-only MP3: ${OUTPUT_PATH}`);
  console.log("[SEVEN_HABITS_FAITH] Done");
}

main().catch((error) => {
  console.error("[SEVEN_HABITS_FAITH] Failed:", error);
  process.exit(1);
});
