...
type BookName = "Job" | "Psalms" | "Proverbs" | "Ecclesiastes" | "Song of Solomon" | "Isaiah" | "Jeremiah" | "Lamentations" | "Ezekiel" | "Daniel" | "Hosea";
...
const ezekielChapters = Array.from({ length: 36 }, (_, index) => index + 13);
const danielChapters = Array.from({ length: 12 }, (_, index) => index + 1);
const hoseaChapters = Array.from({ length: 12 }, (_, index) => index + 1);
...
function trimPhrase(raw: string) {
  ...
  const specials = [
    ...
    "I will make them know mine hand",
...
function category(book: BookName, phrase: string) {
  ...
  if (book === "Lamentations") {
    if (/\b(solitary|weepeth|tears|affliction|desolate|sorrow|grief|mourning)\b/.test(lower)) return "lamentationsGrief";
    if (/\b(sin|transgression|rebellion|unclean|iniquity|punished)\b/.test(lower)) return "lamentationsSin";
    if (/\b(mercies|faithfulness|portion|hope|compassions|wait|salvation)\b/.test(lower)) return "lamentationsHope";
    if (/\b(jerusalem|zion|daughter|gates|sanctuary|city)\b/.test(lower)) return "lamentationsCity";
    if (/\b(turn|remember|behold|restore|prayer)\b/.test(lower)) return "lamentationsPrayer";
    return "lamentationsDetail";
  }

  if (book === "Ezekiel") {
    if (/\b(visions|glory|likeness|cherubims|wheels|firmament|throne|fire)\b/.test(lower)) return "ezekielGlory";
    if (/\b(son of man|word of the lord|spirit|sent|watchman|speak|shepherds|shepherd)\b/.test(lower)) return "ezekielCall";
    if (/\b(rebellious|idols|abominations|iniquity|sin|evil)\b/.test(lower)) return "ezekielRebellion";
    if (/\b(sign|roll|eat|lie|left side|right side|mark|foreheads|valley|dry bones|stick)\b/.test(lower)) return "ezekielSign";
    if (/\b(jerusalem|mountains of israel|temple|sanctuary|house of israel|land of israel)\b/.test(lower)) return "ezekielJerusalem";
    if (/\b(end|judgment|scatter|sword|famine|pestilence|destroy|nations|tyrus|egypt|pharaoh|gog)\b/.test(lower)) return "ezekielJudgment";
    if (/\b(new heart|new spirit|clean water|everlasting covenant|glory of the lord|living waters|inheritance)\b/.test(lower)) return "ezekielRestoration";
    return "ezekielDetail";
  }

  if (book === "Daniel") {
    if (/\b(babylon|king|nebuchadnezzar|belshazzar|darius|decree|palace|province)\b/.test(lower)) return "danielEmpire";
    if (/\b(purposed in his heart|defile|prayer|knees|windows|excellent spirit|faithful)\b/.test(lower)) return "danielFaith";
    if (/\b(dream|vision|image|beasts|horns|weeks|interpretation|mystery)\b/.test(lower)) return "danielVision";
    if (/\b(god of heaven|most high|ancient of days|son of man|kingdom|dominion)\b/.test(lower)) return "danielGod";
    if (/\b(furnace|lion|den|deliver|shut the lions|angel)\b/.test(lower)) return "danielDeliverance";
    return "danielDetail";
  }

  if (book === "Hosea") {
    if (/\b(harlot|whoredom|wife|children|jezreel|lo-ruhamah|lo-ammi)\b/.test(lower)) return "hoseaSign";
    if (/\b(return|heal|mercy|love|betroth|restore|revive|forgive)\b/.test(lower)) return "hoseaMercy";
    if (/\b(knowledge|covenant|law|truth|priest|word|prophet)\b/.test(lower)) return "hoseaKnowledge";
    if (/\b(israel|ephraim|samaria|judah|baal|idols|calves|assyria|egypt)\b/.test(lower)) return "hoseaIsrael";
    if (/\b(judgment|iniquity|sin|transgression|sow|reap|destroyed)\b/.test(lower)) return "hoseaJudgment";
    return "hoseaDetail";
  }
...
function phraseIcon(book: BookName, phrase: string) {
  ...
  if (book === "Lamentations") {
    return {
      ...
      lamentationsGrief: string;
      lamentationsSin: string;
      lamentationsHope: string;
      lamentationsCity: string;
      lamentationsPrayer: string;
      lamentationsDetail: string;
    }];
  }
  ...
  if (book === "Ezekiel") {
    return {
      ...
      ezekielGlory: string;
      ezekielCall: string;
      ezekielRebellion: string;
      ezekielSign: string;
      ezekielJerusalem: string;
      ezekielJudgment: string;
      ezekielRestoration: string;
      ezekielDetail: string;
    }];
  }
  ...
  if (book === "Hosea") {
    return {
      ...
      hoseaSign: string;
      hoseaMercy: string;
      hoseaKnowledge: string;
      hoseaIsrael: string;
      hoseaJudgment: string;
      hoseaDetail: string;
    }];
  }
...
function scorePhrase(book: BookName, phrase: string) {
  ...
  if (book === "Job") {
    ...
    if (/\b(how doth the city|weepeth sore|mercies|great is thy faithfulness|lord is my portion|turn thou us|mine eye|is it nothing)\b/.test(lower)) score += 36;
    if (/\b(zion|jerusalem|affliction|tears|grief|sin|hope|portion|remember)\b/.test(lower)) score += 18;
  } else if (book === "Ezekiel") {
    if (/\b(visions of god|glory of the lord|son of man|watchman|dry bones|new heart|new spirit|clean water|valley full of bones|living waters|glory of the lord came)\b/.test(lower)) score += 36;
    if (/\b(wheels|cherubims|spirit|jerusalem|house of israel|abominations|end|judgment|mountains|shepherds|sanctuary|gog)\b/.test(lower)) score += 18;
  } else if (book === "Daniel") {
    if (/\b(purposed in his heart|god of heaven|stone was cut out|ancient of days|son of man|seventy weeks|excellent spirit|shut the lions mouths)\b/.test(lower)) score += 38;
    if (/\b(babylon|king|dream|vision|mystery|kingdom|dominion|furnace|den|prayer|interpretation)\b/.test(lower)) score += 18;
  } else {
    if (/\b(lo-ammi|not my people|i will allure her|betroth thee unto me|my people are destroyed for lack of knowledge|i desired mercy|out of egypt have i called my son|how shall i give thee up)\b/.test(lower)) score += 38;
    if (/\b(return|mercy|knowledge|ephraim|israel|judah|baal|harlot|whoredom|covenant|idols|heal)\b/.test(lower)) score += 18;
  }
...
function explainLamentations(phrase: string) {
  const lower = phrase.toLowerCase();
  if (/how doth the city|city sit solitary|weepeth sore|is it nothing|mine eye runneth|daughter of zion/.test(lower)) {
    return note([
      `${phrase} gives grief a clear shape instead of leaving sorrow nameless.`,
      "Lamentations makes the fall of Jerusalem personal. The city is pictured as lonely, wounded, and full of tears because covenant judgment has become public ruin.",
      "💧 Grief named\n🏙️ City wounded\n⚠️ Sin remembered\n🙏 Sorrow spoken",
      "The phrase helps the reader slow down and feel the human weight of Jerusalem's fall.",
    ]);
  }
  if (/mercies|great is thy faithfulness|lord is my portion|hope|wait quietly|salvation of the lord/.test(lower)) {
    return note([
      `${phrase} places hope inside suffering without pretending the suffering is small.`,
      "The speaker is still surrounded by affliction, yet he remembers that the LORD's mercy has not run out and His faithfulness has not failed.",
      "🌱 Hope remembered\n🙏 Mercy received\n🕯️ Faithfulness trusted\n⏳ Waiting learned",
      "The phrase helps a beginner see that biblical hope can speak from the middle of pain.",
    ]);
  }
  if (/turn thou us|remember o lord|behold our reproach|renew our days/.test(lower)) {
    return note([
      `${phrase} turns sorrow into prayer instead of leaving it as complaint alone.`,
      "The book does not end by pretending Jerusalem can repair itself. It asks the LORD to see, turn, restore, and renew His people.",
      "🙏 Prayer lifted\n👁️ God asked to see\n🧭 Turning needed\n🌱 Renewal sought",
      "The phrase helps the reader understand restoration as something received from God, not produced by grief itself.",
    ]);
  }
  if (/jerusalem|zion|sanctuary|gates|princes|priests/.test(lower)) {
    return note([
      `${phrase} keeps the reader inside the ruined city rather than in an abstract lesson about sadness.`,
      "Lamentations names places, leaders, gates, priests, and people so the reader understands that judgment touched the whole life of Jerusalem.",
      "🏙️ City named\n⛩️ Sanctuary grieved\n👥 People affected\n⚖️ Judgment felt",
      "The phrase helps the reader connect the poetry to the actual collapse of Jerusalem.",
    ]);
  }

  const kind = category("Lamentations", phrase);
  const openings: Record<string, string> = {
    lamentationsGrief: `${phrase} gives sorrow a direct voice before God.`,
    lamentationsSin: `${phrase} shows that Jerusalem's pain is tied to covenant rebellion.`,
    lamentationsHope: `${phrase} keeps hope alive while the grief remains honest.`,
    lamentationsCity: `${phrase} anchors the lament in Jerusalem's real ruin.`,
    lamentationsPrayer: `${phrase} turns the broken scene toward the LORD in prayer.`,
    lamentationsDetail: `${phrase} gives the reader one exact line to follow inside the lament.`,
  };
  const second: Record<string, string> = {
    lamentationsGrief: "The book teaches readers to name pain truthfully instead of rushing past it.",
    lamentationsSin: "The sorrow is not random. Lamentations remembers the sin that brought judgment.",
    lamentationsHope: "Hope appears because God's mercy and faithfulness are larger than the city's ruin.",
    lamentationsCity: "The city language keeps the reader focused on a people, a place, and a covenant history.",
    lamentationsPrayer: "Prayer becomes the right way to carry grief, guilt, and longing back to God.",
    lamentationsDetail: "The wording may be small, but it helps the reader stay close to the poem's movement.",
  };
  const list: Record<string, string> = {
    lamentationsGrief: "💧 Pain named\n🏙️ Ruin felt\n🕯️ Hope sought\n🙏 Grief spoken",
    lamentationsSin: "⚠️ Sin remembered\n⚖️ Judgment received\n💔 Wound exposed\n🧭 Return needed",
    lamentationsHope: "🌱 Mercy remains\n🕯️ Faithfulness trusted\n🙏 Portion confessed\n⏳ Waiting learned",
    lamentationsCity: "🏙️ Jerusalem mourned\n⛩️ Sanctuary grieved\n👥 People affected\n⚖️ Fall understood",
    lamentationsPrayer: "🙏 LORD addressed\n👁️ Reproach seen\n🧭 Hearts turned\n🌱 Days renewed",
    lamentationsDetail: "🔎 Phrase noticed\n📖 Line followed\n🧭 Meaning located\n🙏 Lament understood",
  };
  const closing: Record<string, string> = {
    lamentationsGrief: "The phrase helps a beginner see that Scripture gives faithful language for sorrow.",
    lamentationsSin: "The phrase keeps grief connected to the seriousness of rebellion against God.",
    lamentationsHope: "The phrase shows that hope can stand inside lament without becoming denial.",
    lamentationsCity: "The phrase helps the reader remember that Jerusalem's fall was a real covenant disaster.",
    lamentationsPrayer: "The phrase teaches the reader to bring brokenness back to the LORD.",
    lamentationsDetail: "The phrase keeps the reader close to Lamentations' actual words.",
  };

  return note([openings[kind], second[kind], list[kind], closing[kind]]);
}

function explainEzekiel(phrase: string) {
  const lower = phrase.toLowerCase();
  if (/heavens were opened|visions of god|likeness of the glory|wheels|cherubims|throne|firmament/.test(lower)) {
    return note([
      `${phrase} introduces Ezekiel to the overwhelming glory of God in exile.`,
      "The vision shows that the LORD is not trapped in Jerusalem or defeated by Babylon. His throne, glory, and living creatures reveal divine rule beyond Israel's collapse.",
      "🔥 Glory revealed\n👑 Throne seen\n🌍 God rules beyond exile\n🙏 Prophet humbled",
      "The phrase helps a beginner see why Ezekiel falls down and listens before he speaks.",
    ]);
  }
  if (/son of man|stand upon thy feet|eat this roll|watchman|speak with my words|sent thee/.test(lower)) {
    return note([
      `${phrase} shows Ezekiel receiving a hard calling from the LORD.`,
      "Ezekiel is not sent to invent a message. He must receive God's words, internalize them, and speak them to a stubborn people.",
      "📣 Prophet sent\n📜 Words received\n👂 People warned\n🛡️ Watchman faithful",
      "The phrase helps the reader understand prophetic ministry as obedience under God's command.",
    ]);
  }
  if (/rebellious house|impudent children|hardhearted|abominations|idols|detestable things/.test(lower)) {
    return note([
      `${phrase} names the resistance Ezekiel is sent to confront.`,
      "The people have religious history, but their hearts and practices are rebellious. Ezekiel's warnings expose the gap between identity and obedience.",
      "⚠️ Rebellion named\n💔 Hearts hardened\n🛑 Idols exposed\n📣 Warning sent",
      "The phrase helps a beginner see why Ezekiel's message is severe and repeated.",
    ]);
  }
  if (/lie thou|left side|right side|set thy face|mark upon the foreheads|dig thou through the wall/.test(lower)) {
    return note([
      `${phrase} turns Ezekiel's message into a visible sign the people cannot easily ignore.`,
      "The prophet acts out judgment, siege, exile, or preservation so the message is seen as well as heard.",
      "📜 Sign acted\n👁️ Message seen\n⚖️ Judgment pictured\n🧭 Meaning made plain",
      "The phrase helps the reader understand why Ezekiel sometimes teaches through symbolic actions.",
    ]);
  }
  if (/glory of the lord went up|cherubims lift up|threshold of the house|east gate/.test(lower)) {
    return note([
      `${phrase} shows the terrifying movement of God's glory away from the polluted temple scene.`,
      "Ezekiel is watching more than architecture. The vision shows how deeply idolatry has offended the holy presence of the LORD.",
      "🔥 Glory moves\n⛩️ Temple judged\n⚠️ Abominations exposed\n🙏 Holiness feared",
      "The phrase helps a beginner see that God's presence must not be treated casually.",
    ]);
  }
  if (/end is come|scatter|sword|famine|pestilence|mountains of israel|jerusalem/.test(lower)) {
    return note([
      `${phrase} announces judgment as the result of long rebellion, not sudden mood.`,
      "Ezekiel ties sword, famine, scattering, and ruin to Israel's stubborn idolatry and refusal to hear the LORD.",
      "⚖️ Judgment announced\n🏙️ Jerusalem warned\n⚠️ Idols confronted\n🧭 Cause made clear",
      "The phrase helps the reader connect Ezekiel's hard images to covenant accountability.",
    ]);
  }
  if (/new heart|new spirit|clean water|dry bones|valley full of bones|one shepherd|everlasting covenant|living waters|glory of the lord came/.test(lower)) {
    return note([
      `${phrase} shows restoration as something only the LORD can give.`,
      "Ezekiel does not picture Israel repairing itself by willpower. God must cleanse, breathe life, gather His people, and bring His glory back.",
      "🌱 Life restored\n💧 Cleansing given\n🕊️ Spirit promised\n👑 God acts first",
      "The phrase helps a beginner see that Ezekiel's hope is not optimism; it is God's power to renew what judgment exposed.",
    ]);
  }

  const kind = category("Ezekiel", phrase);
  const openings: Record<string, string> = {
    ezekielGlory: `${phrase} draws the reader toward the glory and throne-rule of God.`,
    ezekielCall: `${phrase} shows Ezekiel being addressed, sent, or made responsible to speak.`,
    ezekielRebellion: `${phrase} names the rebellion Ezekiel must confront.`,
    ezekielSign: `${phrase} turns the message into something visible and unforgettable.`,
    ezekielJerusalem: `${phrase} keeps the prophecy connected to Jerusalem and Israel's house.`,
    ezekielJudgment: `${phrase} warns that judgment has come because rebellion has continued.`,
    ezekielRestoration: `${phrase} points to the LORD restoring life, worship, and hope after judgment.`,
    ezekielDetail: `${phrase} gives the reader one exact detail for following Ezekiel carefully.`,
  };
  const second: Record<string, string> = {
    ezekielGlory: "Ezekiel begins with God enthroned, holy, mobile, and sovereign even among exiles.",
    ezekielCall: "The prophet must receive the word before he speaks it, and he must speak whether people listen or refuse.",
    ezekielRebellion: "The language is severe because the people keep resisting the LORD while carrying His name.",
    ezekielSign: "Symbolic actions make the warning public, embodied, and hard to dismiss.",
    ezekielJerusalem: "The city and the house of Israel remain central because covenant unfaithfulness has brought covenant judgment.",
    ezekielJudgment: "The warning explains the cause of disaster so the reader does not treat it as random violence.",
    ezekielRestoration: "The restoration language matters because Ezekiel's hope depends on God's action, not Israel's strength.",
    ezekielDetail: "The detail may feel strange at first, but it belongs to Ezekiel's careful prophetic vision.",
  };
  const list: Record<string, string> = {
    ezekielGlory: "🔥 Glory seen\n👑 Throne revealed\n🌍 God rules beyond exile\n🙏 Prophet humbled",
    ezekielCall: "📣 Prophet addressed\n📜 Word received\n👂 People warned\n🛡️ Watchman faithful",
    ezekielRebellion: "⚠️ Rebellion named\n💔 Hearts hardened\n🛑 Idols exposed\n📣 Warning sent",
    ezekielSign: "📜 Sign acted\n👁️ Message seen\n⚖️ Judgment pictured\n🧭 Meaning made plain",
    ezekielJerusalem: "🏙️ Jerusalem named\n⛩️ Sanctuary weighed\n👥 Israel addressed\n⚖️ Covenant judged",
    ezekielJudgment: "⚖️ Judgment announced\n🔥 Sin exposed\n🏙️ City warned\n🧭 Cause made clear",
    ezekielRestoration: "🌱 Life restored\n💧 Cleansing given\n🕊️ Spirit promised\n👑 God acts first",
    ezekielDetail: "🔎 Phrase noticed\n📖 Vision followed\n🧭 Meaning located\n🙏 Reader slowed",
  };
  const closing: Record<string, string> = {
    ezekielGlory: "The phrase helps a beginner see that Ezekiel starts with God's majesty before Israel's ruin.",
    ezekielCall: "The phrase teaches that Ezekiel speaks because God commands him.",
    ezekielRebellion: "The phrase helps the reader understand the resistance behind Ezekiel's warnings.",
    ezekielSign: "The phrase shows why Ezekiel's message often comes through visible actions.",
    ezekielJerusalem: "The phrase keeps the reader connected to the people and place under judgment.",
    ezekielJudgment: "The phrase connects disaster to persistent rebellion against the LORD.",
    ezekielRestoration: "The phrase helps the reader see restoration as God's renewing work after judgment.",
    ezekielDetail: "The phrase keeps the reader close to Ezekiel's actual text.",
  };

  return note([openings[kind], second[kind], list[kind], closing[kind]]);
}

function explainDaniel(phrase: string) {
  const lower = phrase.toLowerCase();
  if (/purposed in his heart|would not defile himself|three times a day|windows being open|excellent spirit/.test(lower)) {
    return note([
      `${phrase} shows faithfulness taking shape before pressure becomes public.`,
      "Daniel's loyalty is not loud at first. It appears in food, prayer, discipline, and a steady refusal to let Babylon define obedience.",
      "🙏 Quiet conviction\n🏛️ Pressure in exile\n📖 Loyalty practiced\n🛡️ Faith held firm",
      "The phrase helps a beginner see that Daniel's courage begins with daily faithfulness.",
    ]);
  }
  if (/god of heaven|most high|ancient of days|son of man|kingdom shall not be destroyed|dominion/.test(lower)) {
    return note([
      `${phrase} lifts the reader above earthly empires to God's lasting rule.`,
      "Daniel keeps showing kings rising and falling, but the God of heaven rules over all kingdoms and gives dominion that no empire can erase.",
      "👑 God reigns\n🏛️ Empires pass\n🌙 Vision opens\n🕊️ Kingdom endures",
      "The phrase helps the reader understand Daniel's main comfort: history is not finally controlled by proud rulers.",
    ]);
  }
  if (/dream|vision|image|beasts|horns|seventy weeks|interpretation|mystery/.test(lower)) {
    return note([
      `${phrase} introduces symbolic revelation that needs God's interpretation.`,
      "Daniel's visions are not puzzles for curiosity alone. They reveal conflict, kingdoms, judgment, and hope under God's control.",
      "🌙 Vision given\n🔎 Mystery opened\n🏛️ Kingdoms weighed\n👑 God explains",
      "The phrase helps a beginner slow down and ask what God is revealing, not just what the symbols look like.",
    ]);
  }
  if (/burning fiery furnace|den of lions|angel|delivered|shut the lions/.test(lower)) {
    return note([
      `${phrase} shows deliverance inside a real threat, not safety from ever being threatened.`,
      "Daniel and his friends face fire, lions, and royal pressure. God is able to rescue, but their faithfulness is real before the rescue appears.",
      "🔥 Trial faced\n🦁 Danger real\n🛡️ God delivers\n🙏 Faith remains",
      "The phrase helps the reader see courage as obedience under threat.",
    ]);
  }

  const kind = category("Daniel", phrase);
  const openings: Record<string, string> = {
    danielEmpire: `${phrase} places the story inside the pressure of foreign empire.`,
    danielFaith: `${phrase} shows covenant loyalty being practiced in exile.`,
    danielVision: `${phrase} introduces a dream or vision that reveals more than human politics can see.`,
    danielGod: `${phrase} points to the God who rules above every kingdom.`,
    danielDeliverance: `${phrase} shows God's power to preserve His servants under threat.`,
    danielDetail: `${phrase} gives the reader one exact detail for following Daniel carefully.`,
  };
  const second: Record<string, string> = {
    danielEmpire: "Babylon and later rulers are powerful, but Daniel keeps showing that their power is limited.",
    danielFaith: "Faithfulness in Daniel is practical: food, prayer, speech, patience, and refusing idolatry.",
    danielVision: "The visions stretch beyond Daniel's lifetime and show God ruling over history.",
    danielGod: "Daniel's God is not a local idol. He rules heaven, kings, beasts, time, and kingdoms.",
    danielDeliverance: "Deliverance does not make the danger fake. It shows God is present in the danger.",
    danielDetail: "The wording keeps the reader attached to the story instead of flattening Daniel into a slogan.",
  };
  const list: Record<string, string> = {
    danielEmpire: "🏛️ Empire named\n👑 Kings limited\n⚖️ Pride weighed\n🙏 Faith tested",
    danielFaith: "🙏 Loyalty practiced\n📖 Conviction held\n🧭 Exile navigated\n🛡️ Courage formed",
    danielVision: "🌙 Vision given\n🔎 Mystery opened\n🏛️ Kingdoms weighed\n👑 God explains",
    danielGod: "👑 God reigns\n⏳ Time ruled\n🏛️ Empires pass\n🕊️ Kingdom endures",
    danielDeliverance: "🔥 Trial faced\n🦁 Danger real\n🛡️ God delivers\n🙏 Faith remains",
    danielDetail: "🔎 Phrase noticed\n📖 Story followed\n🧭 Meaning located\n🙏 Reader slowed",
  };
  const closing: Record<string, string> = {
    danielEmpire: "The phrase helps a beginner read Daniel as faithful life under empire.",
    danielFaith: "The phrase shows that Daniel's courage grows from practiced loyalty.",
    danielVision: "The phrase teaches the reader to let God interpret history.",
    danielGod: "The phrase keeps Daniel centered on God's rule instead of human fear.",
    danielDeliverance: "The phrase helps the reader see rescue as God's power in real danger.",
    danielDetail: "The phrase keeps the reader close to Daniel's actual words.",
  };

  return note([openings[kind], second[kind], list[kind], closing[kind]]);
}

function explainHosea(phrase: string) {
  const lower = phrase.toLowerCase();
  if (/wife of whoredoms|children of whoredoms|lo-ruhamah|lo-ammi|not my people|jezreel/.test(lower)) {
    return note([
      `${phrase} turns Hosea's family sign into a painful picture of Israel's covenant unfaithfulness.`,
      "The names and marriage imagery are not random drama. They show how deeply Israel's idolatry has wounded the covenant relationship.",
      "💔 Covenant pain\n👨‍👩‍👧 Sign family\n⚠️ Idolatry exposed\n🧭 Israel confronted",
      "The phrase helps a beginner understand why Hosea feels so personal and severe.",
    ]);
  }
  if (/i will allure her|betroth thee unto me|i will heal their backsliding|return unto the lord|how shall i give thee up/.test(lower)) {
    return note([
      `${phrase} shows mercy reaching toward people who have been unfaithful.`,
      "Hosea does not soften Israel's sin, but the LORD still speaks of healing, drawing back, betrothing, and restoring His people.",
      "🌱 Mercy offered\n💔 Unfaithfulness named\n🧭 Return called\n🙏 Love remains",
      "The phrase helps the reader see that Hosea's hope is covenant mercy, not denial of sin.",
    ]);
  }
  if (/lack of knowledge|knowledge of god|i desired mercy|not sacrifice|covenant|law/.test(lower)) {
    return note([
      `${phrase} explains that Israel's problem is not ignorance alone but covenant rejection.`,
      "Hosea connects knowledge, mercy, law, and loyalty. The people know religious activity, but they are missing faithful love toward the LORD.",
      "📖 Knowledge rejected\n🙏 Mercy desired\n⚖️ Covenant weighed\n🧭 Loyalty needed",
      "The phrase helps a beginner see why worship without faithful love is exposed in Hosea.",
    ]);
  }
  if (/ephraim|samaria|baal|calves|assyria|egypt|israel slideth back/.test(lower)) {
    return note([
      `${phrase} names the places, powers, and idols pulling Israel away from the LORD.`,
      "Hosea keeps naming Ephraim, Samaria, Baal, Assyria, and Egypt because Israel's unfaithfulness has real political and spiritual forms.",
      "🧭 Israel named\n🛑 Idols exposed\n🏛️ Empires trusted\n🙏 LORD forsaken",
      "The phrase helps the reader connect Hosea's warnings to Israel's actual choices.",
    ]);
  }

  const kind = category("Hosea", phrase);
  const openings: Record<string, string> = {
    hoseaSign: `${phrase} makes Israel's covenant unfaithfulness painfully visible.`,
    hoseaMercy: `${phrase} shows the LORD calling, healing, or restoring His people.`,
    hoseaKnowledge: `${phrase} explains what faithful covenant knowledge should have produced.`,
    hoseaIsrael: `${phrase} names Israel's actual drift among idols, places, and foreign powers.`,
    hoseaJudgment: `${phrase} warns that unfaithfulness brings real covenant consequences.`,
    hoseaDetail: `${phrase} gives the reader one exact detail for following Hosea carefully.`,
  };
  const second: Record<string, string> = {
    hoseaSign: "Hosea uses family and name signs so readers feel the wound of idolatry, not just define it.",
    hoseaMercy: "Mercy in Hosea is costly and persistent because the LORD keeps calling unfaithful people back.",
    hoseaKnowledge: "The issue is relational loyalty to God, not merely having religious information.",
    hoseaIsrael: "The names keep the prophecy grounded in Israel's real worship, politics, and rebellion.",
    hoseaJudgment: "Judgment comes because the people keep sowing covenant betrayal and trusting false help.",
    hoseaDetail: "The wording helps the reader follow Hosea's emotional and covenant logic.",
  };
  const list: Record<string, string> = {
    hoseaSign: "💔 Covenant pain\n👨‍👩‍👧 Sign family\n⚠️ Idolatry exposed\n🧭 Israel confronted",
    hoseaMercy: "🌱 Mercy offered\n💔 Sin named\n🧭 Return called\n🙏 Love remains",
    hoseaKnowledge: "📖 Knowledge rejected\n🙏 Mercy desired\n⚖️ Covenant weighed\n🧭 Loyalty needed",
    hoseaIsrael: "🧭 Israel named\n🛑 Idols exposed\n🏛️ Empires trusted\n🙏 LORD forsaken",
    hoseaJudgment: "⚖️ Judgment warned\n🌾 Sowing shown\n🔥 Sin exposed\n🧭 Return needed",
    hoseaDetail: "🔎 Phrase noticed\n📖 Oracle followed\n🧭 Meaning located\n🙏 Reader slowed",
  };
  const closing: Record<string, string> = {
    hoseaSign: "The phrase helps a beginner feel why Hosea treats idolatry as betrayal.",
    hoseaMercy: "The phrase shows that Hosea's mercy is honest about sin and strong enough to heal.",
    hoseaKnowledge: "The phrase teaches that knowing God should create mercy, loyalty, and obedience.",
    hoseaIsrael: "The phrase helps the reader see Israel's drift in concrete terms.",
    hoseaJudgment: "The phrase keeps judgment connected to repeated covenant betrayal.",
    hoseaDetail: "The phrase keeps the reader close to Hosea's actual text.",
  };
...
function explain(book: BookName, phrase: string) {
  ...
  if (book === "Lamentations") return explainLamentations(phrase);
  if (book === "Ezekiel") return explainEzekiel(phrase);
  if (book === "Daniel") return explainDaniel(phrase);
  if (book === "Hosea") return explainHosea(phrase);
...
async function main() {
  const ezekielSections = await buildBook("Ezekiel", ezekielChapters);
  const danielSections = await buildBook("Daniel", danielChapters);
  const hoseaSections = await buildBook("Hosea", hoseaChapters);

  writeFileSync(
    "lib/ezekielThirteenToFortyEightPersonalNotes.ts",
    `import type { PersonalLeviticusPhraseSectionInput } from "./leviticusOneToTenPersonalNotes";

type EzekielPhraseSectionInput = PersonalLeviticusPhraseSectionInput & { book: "Ezekiel" };

export const EZEKIEL_13_48_PERSONAL_SECTIONS = [
${serialize(ezekielSections)}
] as const satisfies readonly EzekielPhraseSectionInput[];
`,
  );

  writeFileSync(
    "lib/danielOneToTwelvePersonalNotes.ts",
    `import type { PersonalLeviticusPhraseSectionInput } from "./leviticusOneToTenPersonalNotes";

type DanielPhraseSectionInput = PersonalLeviticusPhraseSectionInput & { book: "Daniel" };

export const DANIEL_1_12_PERSONAL_SECTIONS = [
${serialize(danielSections)}
] as const satisfies readonly DanielPhraseSectionInput[];
`,
  );

  writeFileSync(
    "lib/hoseaOneToTwelvePersonalNotes.ts",
    `import type { PersonalLeviticusPhraseSectionInput } from "./leviticusOneToTenPersonalNotes";

type HoseaPhraseSectionInput = PersonalLeviticusPhraseSectionInput & { book: "Hosea" };

export const HOSEA_1_12_PERSONAL_SECTIONS = [
${serialize(hoseaSections)}
] as const satisfies readonly HoseaPhraseSectionInput[];
`,
  );

  console.log(`Wrote ${ezekielSections.length} Ezekiel sections.`);
  console.log(`Wrote ${danielSections.length} Daniel sections.`);
  console.log(`Wrote ${hoseaSections.length} Hosea sections.`);
}
