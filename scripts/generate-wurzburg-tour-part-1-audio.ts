...
const outputPath = join(process.cwd(), "exports", "wurzburg-residence-tour", "Würzburg Residence Tour – Part 1.mp3");
const scriptPath = join(process.cwd(), "exports", "wurzburg-residence-tour", "part-1-narration-script.md");
...
async function generateOpenAiSpeechPcm(text: string) {
  ...
  const response = await fetch("https://api.openai.com/v1/audio/speech", {
    ...
    body: JSON.stringify({
      ...
      input: text,
      instructions:
        "Speak like a warm, friendly, professional tour guide leading a live walking tour group of cruise passengers through a European palace. Relaxed, conversational, confident, and patient, with occasional light warmth or gentle humor, but never theatrical or exaggerated. Clear pronunciation of European names and places, natural pauses between sentences and when moving to a new topic, and a steady, unhurried pace suitable for guests following along on tour headsets.",
    }),
  });
