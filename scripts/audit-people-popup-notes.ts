...
function isLegacyPersonPopup(notes: string) {
  ...
  return /still being filled in|quick meaning|quick anchor|keep reading without getting stuck|fuller explanation|their role in the story|where you find .*|why this person matters/i.test(
    normalized
  );
}
...
async function main() {
  ...
  const items = dedupe(BIBLE_PEOPLE_LIST.map((entry) => entry.name));
  ...
  for (const name of items) {
    ...
    if (isLegacyPersonPopup(existing)) {
      legacy += 1;
