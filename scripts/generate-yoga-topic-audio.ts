...
const MP3_KBPS = 96;
const SCRIPT_PATH = join(process.cwd(), "docs", "is-doing-yoga-a-sin-narrator-script.md");
const OUTPUT_PATH = join(process.cwd(), "public", "audio", "bible-topics", "yoga", "is-doing-yoga-a-sin-narrator-only.mp3");
...
async function generateOpenAiSpeechPcm(text: string) {
  ...
  const response = await fetch("https://api.openai.com/v1/audio/speech", {
    ...
    body: JSON.stringify({
      ...
      input: text,
      instructions:
        "Speak in a deep, warm, calm male Bible Buddy narrator voice. This is a thoughtful Christian explainer video, so keep the tone conversational, clear, biblical, and balanced. Do not sound angry, rushed, robotic, academic, salesy, or like a loud preacher. Let the Scripture references and the careful distinctions breathe naturally.",
      response_format: "pcm",
    }),
  });
...
async function main() {
  ...
  console.log(`[YOGA_TOPIC] Text chars: ${text.length}, chunks: ${chunks.length}`);
  for (const [index, chunk] of chunks.entries()) {
    console.log(`[YOGA_TOPIC] Generating narrator chunk ${index + 1}/${chunks.length}`);
  ...
  console.log(`[YOGA_TOPIC] Local narrator-only MP3: ${OUTPUT_PATH}`);
  console.log("[YOGA_TOPIC] Done");
}

main().catch((error) => {
  console.error("[YOGA_TOPIC] Failed:", error);
  process.exit(1);
});
