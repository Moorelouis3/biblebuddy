...
type BookName = "Job" | "Psalms" | "Proverbs" | "Ecclesiastes" | "Song of Solomon" | "Isaiah" | "Jeremiah" | "Lamentations" | "Ezekiel";
...
const jeremiahChapters = Array.from({ length: 43 }, (_, index) => index + 10);
const lamentationsChapters = Array.from({ length: 5 }, (_, index) => index + 1);
const ezekielChapters = Array.from({ length: 12 }, (_, index) => index + 1);
...
function category(book: BookName, phrase: string) {
  ...
  if (book === "Ezekiel") {
    ...
    if (/\b(son of man|word of the lord|spirit|sent|watchman|speak)\b/.test(lower)) return "ezekielCall";
    if (/\b(rebellious|idols|abominations|iniquity|sin|evil)\b/.test(lower)) return "ezekielRebellion";
    if (/\b(sign|roll|eat|lie|left side|right side|mark|foreheads)\b/.test(lower)) return "ezekielSign";
    if (/\b(jerusalem|mountains of israel|temple|sanctuary|house of israel)\b/.test(lower)) return "ezekielJerusalem";
    if (/\b(end|judgment|scatter|sword|famine|pestilence|destroy)\b/.test(lower)) return "ezekielJudgment";
...
function scorePhrase(book: BookName, phrase: string) {
  ...
  if (book === "Job") {
    ...
    if (/\b(visions of god|glory of the lord|son of man|eat this roll|watchman|left side|mark upon the foreheads|cherubims lift up|rebellious house)\b/.test(lower)) score += 36;
    if (/\b(wheels|cherubims|spirit|jerusalem|house of israel|abominations|end|judgment|mountains)\b/.test(lower)) score += 18;
  }
...
async function main() {
  ...
  const lamentationsSections = await buildBook("Lamentations", lamentationsChapters);
  const ezekielSections = await buildBook("Ezekiel", ezekielChapters);

  writeFileSync(
    "lib/jeremiahTenToFiftyTwoPersonalNotes.ts",
...
export const JEREMIAH_10_52_PERSONAL_SECTIONS = [
...
`,
  ...
  writeFileSync(
    "lib/lamentationsOneToFivePersonalNotes.ts",
    `import type { PersonalLeviticusPhraseSectionInput } from "./leviticusOneToTenPersonalNotes";

type LamentationsPhraseSectionInput = PersonalLeviticusPhraseSectionInput & { book: "Lamentations" };

export const LAMENTATIONS_1_5_PERSONAL_SECTIONS = [
${serialize(lamentationsSections)}
] as const satisfies readonly LamentationsPhraseSectionInput[];
`,
  );

  writeFileSync(
    "lib/ezekielOneToTwelvePersonalNotes.ts",
    `import type { PersonalLeviticusPhraseSectionInput } from "./leviticusOneToTenPersonalNotes";

type EzekielPhraseSectionInput = PersonalLeviticusPhraseSectionInput & { book: "Ezekiel" };

export const EZEKIEL_1_12_PERSONAL_SECTIONS = [
...
`,
  ...
  console.log(`Wrote ${lamentationsSections.length} Lamentations sections.`);
