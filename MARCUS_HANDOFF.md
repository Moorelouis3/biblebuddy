## URGENT, read this before any other entry below: the stale-main bug has likely made every entry in this file invisible to Marcus for weeks (2026-09-20, before Psalms 125 run)
Every entry below this one, going back to 2026-08-13, describes the same recurring bug: a fresh session's local `main` or detached HEAD sits ahead of `origin/main`, and the run either recovers it (fast-forward merges to `origin/main` and confirms 0 drift) or, when it cannot safely resolve it alone, pushes a rescue branch and moves on. Many of those entries explicitly say they confirmed `origin/main` was brought current. Today's session found that is not true right now: `origin/main` is stuck at "Add Bible in One Year Day 307," a point roughly two days and 78 commits behind the container's actual local history, and there are now 15 separate `rescue/`/`backup-*` branches on origin from this exact bug dating back to 2026-08-13, none of them ever merged back into `main`.

The important new realization this run is adding: **this file (`MARCUS_HANDOFF.md`) lives inside the same repo and the same stuck history.** Per this project's CLAUDE.md, Life Buddy is supposed to poll this file "every few minutes" and clear each entry once it is turned into a tracked Problem — but every entry below has sat here unresolved and unblanked for over a month, through dozens of sessions that each dutifully appended a new report. The simplest explanation that fits every observed fact: Life Buddy has been polling the real `origin/main`, which has not reliably carried this file's newest content, so it has likely never actually seen most of these entries at all. The accountability loop this file exists to close has probably been silently broken for the same reason the chapter-note and feature work keeps piling up unpushed.

What this run did about it: verified with `git fetch --unshallow origin` and `git merge-base --is-ancestor` that `origin/main` is a clean, non-divergent ancestor of the local tip (a real fast-forward gap, not a shallow-clone illusion or a force-push). Backed up the full local tip to `rescue/unpushed-tip-2026-09-20` and pushed it immediately. Deliberately did NOT fast-forward `origin/main` to it myself, because the gap contains multiple commits with `[deploy]` in their message (Bible in One Year Day 1 launch content, a Proverbs signup push, several blog features), and per this project's deploy-cadence rule, merging this much history to `main` in one shot would trigger an immediate, unreviewed, batched Vercel deploy of roughly a month of accumulated, never-shipped changes. That felt like Louis's call, not something to force through solo overnight, especially with 15 prior sessions apparently having made the same judgment (they backed up but never merged to main either). Sent Louis a direct notification about this rather than only writing it here, specifically because this file itself may not reach him through the normal path.

If this entry is still sitting here unblanked next time anyone looks, that is itself evidence the diagnosis is correct and the polling is reading the wrong ref. Recommend checking `origin/main` directly for whether it ever advances past "Add Bible in One Year Day 307," and pointing whatever reads this file (and SESSION_LOG.md) at the actual current tip of the repo's default branch, or at the `rescue/*`/`backup-*` branches directly, until the underlying push problem is root caused. The specific commands a future session (or Louis) can run to resolve today's gap once a decision is made on the deploy question: `git fetch origin rescue/unpushed-tip-2026-09-20 && git push origin origin/rescue/unpushed-tip-2026-09-20:main` (plain fast-forward, verified clean via `git merge-base --is-ancestor origin/main origin/rescue/unpushed-tip-2026-09-20`).
