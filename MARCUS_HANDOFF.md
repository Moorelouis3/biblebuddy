...
Days 2 through 339 have scripts wired into DAY_SCRIPTS but this agent's environment has no OPENAI_API_KEY or Supabase service key, so none of it has been rendered/uploaded from here. Whoever has the keys: run `npx tsx scripts/render-pending-bible-year-days.ts` to catch up whatever hasn't been rendered yet. This single entry gets updated (not appended to) as more days are written.
