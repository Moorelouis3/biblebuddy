...
export default function BibleTopicsPanel({ userId }: BibleTopicsPanelProps) {
  ...
  function toggleLessonCard(lessonSlug: string) {
    setOpenLessonSlug((current) => {
      const next = current === lessonSlug ? current : lessonSlug;
  ...
  if (selectedLessonOpen && selectedTopic && selectedLesson) {
    return (
        ...
        <div className="mx-auto flex max-w-xl flex-col gap-4 pb-7">
          <article className="bb-skin-glow-card overflow-hidden rounded-[28px] border border-[var(--bb-card-border,#dbe7f4)] bg-[var(--bb-card,#ffffff)] text-left shadow-[0_14px_36px_rgba(38,63,99,0.10)]">
            <div className="relative bg-[radial-gradient(circle_at_18%_0%,color-mix(in_srgb,var(--bb-accent,#2f7fe8)_22%,transparent),transparent_42%),linear-gradient(135deg,color-mix(in_srgb,var(--bb-card,#ffffff)_88%,transparent),color-mix(in_srgb,var(--bb-surface-soft,#f8fbff)_68%,transparent))] px-4 pb-5 pt-5 sm:px-5">
              ...
              <p className="pr-12 text-xs font-black uppercase tracking-[0.16em] text-[var(--bb-accent,#2f7fe8)]">{selectedTopic.title} Lesson</p>
              <h1 className="mt-1 pr-12 text-3xl font-black leading-tight text-[var(--bb-text-primary,#111827)]">{selectedLesson.title}</h1>
              ...
              {selectedLessonVideoSrc ? (
                <div className="overflow-hidden rounded-[26px] border border-[color-mix(in_srgb,var(--bb-accent,#f6b44b)_36%,var(--bb-card-border,#dbe7f4))] bg-[var(--bb-card,#ffffff)] shadow-[0_18px_42px_rgba(0,0,0,0.16)]">
                  <div className="border-b border-[color-mix(in_srgb,var(--bb-accent,#f6b44b)_22%,var(--bb-card-border,#dbe7f4))] bg-[linear-gradient(135deg,color-mix(in_srgb,var(--bb-card,#ffffff)_88%,transparent),color-mix(in_srgb,var(--bb-surface-soft,#f8fbff)_76%,transparent))] px-4 py-3">
                    ...
                    <h2 className="mt-1 text-lg font-black leading-tight text-[var(--bb-text-primary,#111827)]">{selectedLesson.title}</h2>
                  </div>
                  <div className="p-3">
