...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await ensureWeeklyGroupPollPost(supabaseAdmin, targetGroup.id);
    return NextResponse.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not create weekly poll.";
