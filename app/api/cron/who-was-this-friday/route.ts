...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await ensureWeeklyGroupSeriesPost(supabaseAdmin, targetGroup.id, "who_was_this_friday");
    return NextResponse.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not create Who Was This Friday post.";
