...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await ensureWeeklyGroupSeriesPost(supabaseAdmin, targetGroup.id, "update_monday");
    return NextResponse.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not create Update Monday post.";
