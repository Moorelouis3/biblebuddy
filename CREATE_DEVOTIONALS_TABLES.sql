-- Create devotionals tables
-- Devotionals: Main devotional information
-- Devotional days: Individual days within a devotional
-- Devotional progress: User progress tracking
...
-- RLS Policies for devotionals (read-only for all authenticated users)
CREATE POLICY "devotionals_select_all"
ON public.devotionals
FOR SELECT
TO authenticated
USING (true);

-- RLS Policies for devotional_days (read-only for all authenticated users)
CREATE POLICY "devotional_days_select_all"
